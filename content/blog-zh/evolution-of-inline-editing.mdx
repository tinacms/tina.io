...
seo:
  ...
  description: >-
    探索TinaCMS中内联编辑的演变，从早期阶段到当前版本，直接在页面上简化内容更新
title: 内联编辑的演变
...
内联编辑消除了内容编辑者在使用传统解耦CMS表单时面临的许多可用性障碍。编辑者可以立即在其实际网站的上下文中看到内容更改。但开发者们与我们分享了当前内联编辑实现的权衡。

我们正在继续从我们的开发社区中学习，并寻找更好的方法来整合内联编辑赋予的超能力。

在这个讨论中，我们不能忘记侧边栏编辑！内联编辑的吸引力常常掩盖了我们通过侧边栏编辑获得的灵活设置和可用性优势。**我们建议从[侧边栏编辑](/docs/using-tina-editor)开始，以便快速启动和运行。**
...
通过这种方法，我们作为开发者获得了很多控制权。内联编辑组件与您的布局组件一起存在，并且几乎如您所期望的那样表现。
...
* 侧边栏编辑表单是从一个单一的配置对象构建的。内联编辑带来了不同的开发者体验和另一组需要理解的概念。

将内联编辑与您的组件耦合的另一个副作用是，您现在将TinaCMS与您的组件一起带到了各处。

我们可以尝试有条件地添加内联编辑组件，但很明显，包含内联编辑所需的逻辑现在与我们的页面和组件耦合在一起。现在更难以维护、共享或重用这些组件。

根据您的使用情况，内联编辑的可用性优势可能会超过这些顾虑。但我们正在寻找更少权衡的解决方案。

## 替代方法

### 基于引用的内联编辑

`useFieldRef`是一个实验性新API的第一部分，用于在TinaCMS中创建基于引用的内联编辑体验。使用`useFieldRef`，内联编辑组件在表单配置中定义，您可以在布局中将一个[ref](https://react.dev/learn/referencing-values-with-refs#refs-and-the-dom)分配给字段应附加到的组件。

以这种方式创建的内联字段绝对定位在引用组件的顶部，并符合其尺寸。基于引用的内联编辑使字段看起来像是在DOM中替换布局组件，而不改变标记。

与我们的第一个示例相比，我们在表单配置对象中定义一个`inlineComponent`键。我们以设置侧边栏编辑的相同方式添加一个内联编辑组件！
...
如果您担心TinaCMS与您的网站捆绑在一起的性能影响，您可以创建特定于编辑的路由。像[Next.js](https://nextjs.org)这样的框架优化Javascript包，以便TinaCMS*不*包含在不需要的路由中。
