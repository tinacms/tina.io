...
warningMessage: >-
  **更新：** 本文中的示例引用了过时的Gatsby实现。我们建议使用[Next.js](/zh/docs/setup-overview/)来获得更流畅的解决方案。
seo:
...
[TinaCMS](/zh/docs/setup-overview)是一个*可扩展的*工具包，可以帮助满足这些需求。Tina允许开发者使用我们喜爱的格式，如Markdown和JSON，同时为内容作者提供流畅的体验。
...
*Tyra*使用Markdown作为内容，但TinaCMS也通过[gatsby-tinacms-json](/zh/docs/guides/converting-gatsby-to-tina)支持JSON文件。我发现JSON非常适合页面内容和[块](/zh/blog/what-are-blocks)。但对于简单的博客文章，Markdown效果很好。
...
我们可以通过将`FormConfig`对象传递给Tina来配置侧边栏表单。[使用Tina自定义表单](/zh/docs/guides/converting-gatsby-to-tina)很简单。我们需要定义一个JavaScript对象来声明Tina要渲染的所需表单字段。
...
在`FormConfig`中，我们使用`text`、`markdown`、`date`，甚至`image`字段来使文章创作体验更好。Tina有[很多](/zh/docs/reference/fields)内置字段，甚至允许你[添加自己的](/zh/docs/extending-tina/custom-field-components)。
