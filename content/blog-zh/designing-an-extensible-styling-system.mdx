...
所有这些分布在许多包和文件中的组件都需要一个单一的真相来源。颜色、填充大小、阴影、字体……一切都必须在 TinaCMS 组件之间保持一致。Styled Components 提供了一个强大的主题系统，非常适合解决这个问题。我们可以为任何希望使用 TinaCMS 通用样式的组件提供主题上下文。

最初，TinaCMS UI 被包含在一个 iframe 内的隔离侧边栏中。当我们转向使用 TinaCMS 进行内联内容编辑时，特定字段的 UI 将直接显示在用户的网站上，而不是在一个隔离的 TinaCMS 容器中。由于主题对象是在 TinaCMS 内部构建的，为页面内组件提供当前主题是一个挑战。通过 gatsby-config.js 配置的主题更改只能全局进行，并且只有在服务器重启时才会生效。我们需要一些更适合不断发展的 TinaCMS 项目的东西。
