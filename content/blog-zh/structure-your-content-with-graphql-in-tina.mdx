...
seo:
  ...
  description: >-
    探索如何在TinaCMS中使用GraphQL来更高效地构建和查询你的内容，提高数据获取和内容管理。
...
<WarningCallout
  body={<>
    **更新**: TinaCMS API一直在发展，本文中的内容已过时。要获取TinaCMS的入门帮助，我们建议查看我们的[入门文档](https://tina.io/docs/setup-overview/)。
  </>}
/>
...
我们开源了[tina-graphql-gateway包](https://github.com/tinacms/tina-graphql-gateway)，以便你可以了解其工作原理。它包含了不同的工具供开发者使用TinaCMS和GraphQL：

- 一个GraphQL模式编译器，
- 一个GraphQL服务器，
- 一个GraphQL客户端（Altair），
- 一个命令行接口，用于编译模式和启动本地服务器，
- Next.js的TinaCMS助手，
- 以及更多……
...
我们还需要为作者定义一个集合，以及一个基本的作者模板，其中有：
...
TinaCMS CLI提供了一个命令来运行本地GraphQL服务器，以监视我们模式中的更改，并在编辑时编译它；它在开发模式下与Next.js一起运行。我们将其设为`package.json`中的默认`dev` npm脚本，这样当我们运行`yarn dev`时，我们就可以访问GraphQL playground：
...
现在我们知道如何从我们的文件中查询数据，我们仍然需要在我们的Next.js应用程序中使用该查询，从客户端请求它，并要求TinaCMS生成相应的表单，以便我们的贡献者能够直观地编辑它。我们将在后续文章中详细介绍剩余步骤，敬请期待！
