---
seo:
  title: 使用GraphQL在Tina中构建你的内容 | TinaCMS博客
  description: >-
    探索如何在TinaCMS中使用GraphQL来更高效地构建和查询你的内容，提升数据获取和内容管理。
title: 使用GraphQL在Tina中构建你的内容
...
<WarningCallout
  body={<>
    **更新**: Tina API一直在演变，本文中的内容已过时。要获取Tina的入门帮助，我们建议查看我们的[入门文档](https://tina.io/docs/setup-overview/)。
  </>}
/>

Tina采用以开发者为中心的方法来构建内容，这意味着你可以在你喜欢的编辑器中本地建模你的内容。得益于存储在你仓库中的[文件上的GraphQL层](/blog/using-graphql-with-the-filesystem/)，Tina使得在所有文件中查询内容变得更加简单。让我们看看这是什么感觉。

当我们提到基于Git时，这意味着你的内容存储在Markdown和JSON文件中，并通过前置元数据建模；在此基础上，TinaCloud旨在让你利用[GraphQL](https://graphql.org/)的强大功能。

我们开源了[tina-graphql-gateway包](https://github.com/tinacms/tina-graphql-gateway)，以便你可以了解其工作原理。它包含了开发者与Tina和GraphQL一起工作的不同工具：
...
Tina CLI提供了一个命令来运行本地GraphQL服务器，以监视我们模式中的更改，并在编辑时编译它；它在开发模式下与Next.js一起运行。我们将其设为`package.json`中的默认`dev` npm脚本，这样当我们运行`yarn dev`时，我们可以访问GraphQL playground：
...
现在我们知道如何从文件中查询数据，我们仍然需要在我们的Next.js应用程序中使用该查询，从客户端请求它，并要求Tina生成相应的表单，以便我们的贡献者能够直观地编辑它。我们将在后续文章中详细介绍剩余步骤，敬请期待！
