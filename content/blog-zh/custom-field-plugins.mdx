...
插件**扩展了CMS的功能**；字段插件允许我们创建和注册自定义字段。*创建字段插件有几个原因。*通过自定义字段，您可以完全控制编辑体验和功能。如果TinaCMS提供的主要字段不适合您的用例，**您可以填补这些空白**。
...
要跟随学习，您应该已经设置了一个带有TinaCMS表单的自定义字段组件。如果没有，您可以从上一篇文章中获取更多背景信息：[如何创建自定义字段组件](https://tinacms.org/blog/custom-field-components)。在接下来的示例中，我使用了我们上一篇文章中的相同[llama-filters](https://github.com/kendallstrautman/llama-filters) 🦙 演示。
...
让我们进一步解析字段插件。下面的接口应提供一些关于创建字段插件所需内容的见解。当您将字段插件注册到TinaCMS时，它期望一个具有类似结构的对象。
...
**就是这样！** 定义并注册到CMS的插件，您可以无限次重用此字段。在我看来，创建字段插件有助于保持定义表单的一致接口。这样，自定义字段就像本地TinaCMS字段一样在*幕后*工作，这非常酷。

## 简短而甜美，如承诺的那样 🍰

这篇文章结合前一篇，应该为您提供所有构建块，以开始制作自己的字段插件。随时*深入文档*了解[字段](https://tinacms.org/docs/plugins/fields/custom-fields/)或[插件](https://tinacms.org/docs/cms#plugins)。确保与我们[@tinacms](https://twitter.com/tinacms)分享您的炫酷自定义字段🖖。或者，如果您觉得TinaCMS缺少一个基本字段，[打开一个PR](https://github.com/tinacms/tinacms/)来贡献您的自定义字段！