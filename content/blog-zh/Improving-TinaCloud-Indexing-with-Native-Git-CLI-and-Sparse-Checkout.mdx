...
如果你在使用TinaCMS，那么你很可能在使用TinaCloud。当不自托管TinaCMS时，TinaCloud是TinaCMS交互的后端。它提供了诸如搜索文件等有用的功能。为了让TinaCMS能够执行这些操作，你的仓库内容需要被索引。

管理大规模仓库对团队来说是一个独特的挑战。内容索引过程最初是使用`isomorphic-git`实现的，这是一种基于Node.js的Git解决方案。这个方法在大多数情况下运行良好，但随着TinaCMS的用例增长，仓库的规模也在扩大。最近，我们有一个用户在尝试索引其存储数百万文件的仓库时遇到了内存不足（OOM）错误。
...
解决方案的关键在于利用Git的稀疏检出功能，结合克隆命令提供的许多标志，这使我们能够准确指定要包含在工作目录中的文件和目录。我们能够读取TinaCMS配置文件，只拉取索引项目所需的内容。
...
我们很高兴能继续改进TinaCMS，并期待着提供更多关于我们幕后工作的更新。如果你有任何反馈或问题，请随时通过[Discord](https://discord.com/invite/zumN63Ybpf)联系我们的团队，或访问我们的[文档](https://tinacms.org/docs)获取更多信息。