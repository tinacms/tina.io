...
在开发网站时，你可能需要引入国际化，也称为 i18n。虽然 TinaCMS 目前没有提供插件或原生支持，但我们可以利用 Next.js 的 i18n 功能来创建一个支持国际化的网站。
...
## 使用 TinaCMS 为每个语言环境创建文件

为了创建我们的文件，我们将利用 `<locale>/posname` 的文件名结构，语言环境将由 Next.js 处理。因此，文件名 `fr/bonjour` 将在你的 Next.js 应用程序中被视为 `<domain>/fr/another-post`。

下面是文件结构的示例。

![在 TinaCMS 中创建新文件时使用的文件结构示例](/img/blog/i18n/test-blog-post.png)

## 如何跟上 TinaCMS 的更新？

跟上 TinaCMS 的最佳方式是订阅我们的新闻通讯。我们每两周发送一次更新。更新包括新功能、我们正在进行的工作、你可能错过的博客文章等等！

你可以通过点击此链接并输入你的电子邮件来订阅：[/community](/community)
...
我们的 Twitter 账号 ([@tinacms](https://twitter.com/tinacms)) 宣布最新的功能、改进和 TinaCMS 的抢先预览。如果你在项目中标记我们，我们也会很高兴。
