...
TinaCMS提供了A/B测试的能力，使营销团队在实现后无需开发团队即可测试内容。
...
2. 使用TinaCMS配置我们的A/B测试，以便我们的编辑可以启动动态A/B测试。
...
你可以在TinaCMS中这样做：[http://localhost:3000/admin#/collections/page/new](http://localhost:3000/admin#/collections/page/new)
...
让我们通过为其创建一个TinaCMS集合，使我们之前的`content/ab-test/index.json`文件可编辑。
...
> 你可能注意到了`ui`属性。我们使用它来为列表项提供更具描述性的标签。你可以在我们的[扩展TinaCMS文档](https://tina.io/docs/extending-tina/customize-list-ui/)中了解更多。
...
保持与TinaCMS同步的最佳方式是订阅我们的新闻通讯。我们每两周发送一次更新。更新包括新功能、我们正在进行的工作、你可能错过的博客文章等等！
...
我们的Twitter账号（[@tinacms](https://twitter.com/tinacms)）宣布最新的功能、改进和TinaCMS的预览。如果你在项目中使用了我们，我们也会很高兴你能标记我们。