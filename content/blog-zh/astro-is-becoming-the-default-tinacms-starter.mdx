...
seo:
  ...
  description: 在最新的无React可视化编辑路径在真实项目中有更多时间后，Astro将成为默认的TinaCMS入门模板。
  canonicalUrl: '/zh/blog/astro-is-becoming-the-default-tinacms-starter'
...
最新的**Astro**入门模板已经使用了无React的可视化编辑，一旦这种新路径在真实项目中有更多时间，Astro将成为默认选择。

我们尚未更改默认设置。[最新的Astro入门模板已经可用](/zh/roadmap)，我们希望在将Astro作为新项目的主要路径之前，有更多人尝试它。
...
越来越多的TinaCMS用户选择Astro。尽管Next.js仍是默认选择，但入门模板的克隆数量最近已经超过了Next.js入门模板。我们还在[Discord和支持中看到更多Astro问题](https://github.com/tinacms/tinacms/discussions/3399)。这与我们的经验一致：
...
如果您想测试新设置，大多数迁移工作通常在自定义富文本组件中。您`TinaMarkdown` `components`映射中的任何内容可能需要从`.tsx`移动到`.astro`。

其他更改较小：安装`@tinacms/astro`，用`<TinaIsland>`替换`useTina()`，并更新您的数据加载器以适应新的预览流程。

准备好后，将您的项目与[Astro指南](/zh/docs/frameworks/astro)进行比较。
