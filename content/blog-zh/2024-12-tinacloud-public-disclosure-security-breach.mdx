...
2024年12月15日，TinaCMS发现涉及被盗用的AWS访问密钥的未经授权活动。这些密钥被利用来通过我们的Amazon Simple Email Service (SES)基础设施发送未经授权的电子邮件（目标是法国的普通社区，而不是TinaCMS的客户）。
...
外发电子邮件功能，包括用户邀请，受到了影响。目前此问题已解决。
...
2. ✅ 完成 - 访问验证：
   审查CloudTrail日志以识别并确认未经授权的行为者访问的系统
3. ✅ 完成 - 确认安全控制：
...
7. ✅ 完成 - 仓库密钥审计：
   正在对所有GitHub仓库进行彻底审计，以识别在过去的构建或提交中可能无意中暴露的任何其他敏感信息
8. ✅ 完成 - 加强IAM策略：
   已审查并更新与CI/CD系统相关的IAM策略，以确保遵循最小权限原则，移除不必要的权限，特别是那些具有根或管理访问权限的权限
9. ✅ 完成 - AWS访问的IP白名单：
   AWS IAM角色的使用已限制在受信任的IP范围内，特别是对于CI/CD系统和其他敏感操作
10. ✅ 完成 - 持续监控和警报：
    将实施持续监控工具，如Amazon GuardDuty、AWS CloudTrail Insights和AWS Security Hub，以检测和警报可疑活动，如新访问密钥的创建或异常IP访问
11. \[待办] 自动化安全扫描：
    将在CI/CD管道中集成自动化工具，以主动检测代码构建过程中的密钥或漏洞

## 对TinaCMS客户的建议

1. 报告可疑邮件：如果您收到来自TinaCMS的未经授权或可疑邮件，请将其报告至[security@tina.io](mailto\:security@tina.io)
...
* 电子邮件：[security@tina.io](mailto\:security@tina.io)
