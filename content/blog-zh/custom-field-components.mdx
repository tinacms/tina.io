...
表单字段是任何CMS的基础。虽然TinaCMS提供了一套开箱即用的字段集合，但您也可以创建自己的字段。本文将向您展示如何创建自定义字段组件并在TinaCMS侧边栏中使用它们的基本概念。

**先决条件 👩‍🏫**

在整篇文章中，我将提到一些TinaCMS的核心概念，如[表单](https://tinacms.org/docs/plugins/forms)、侧边栏和[字段](https://tinacms.org/docs/plugins/fields)。在阅读之前，了解一些**[TinaCMS的工作原理](https://tinacms.org/docs/getting-started/introduction)**的基本知识会很有帮助。

## 为什么要创建自定义字段？

TinaCMS旨在完全可定制和可扩展。创建**自定义字段可以提供对侧边栏配置和样式的精确控制**，并实现独特的字段功能。

<WebmEmbed embedSrc="/img/blog/custom-field-components/saturate-custom-field.webm" />

> 想要提前了解？可以随时查看上面gif中自定义范围输入字段的**[完成版本](https://github.com/kendallstrautman/llama-filters)**，或者在Tina Grande仓库中看看更复杂的[*Authors*字段插件](https://github.com/tinacms/tina-starter-grande/blob/master/src/fields/authors.js)。

## 两种方法——让我们从简单开始

有两种方法可以将[自定义字段](https://tinacms.org/docs/plugins/fields/custom-fields)添加到TinaCMS。第一种方法是定义一个React组件，并将其传递给字段定义的`component`属性。TinaCMS团队称之为**内联字段组件**。这种方法更简单；这将是本文的重点方法。

第二种方法是定义一个自定义组件，然后将该组件注册为CMS的[字段插件](https://tinacms.org/docs/plugins/fields/custom-fields#2-creating-field-plugins)。TinaCMS提供的所有[核心字段](https://tinacms.org/docs/plugins/fields)都用作插件。

创建插件与内联字段相比有一些优势——主要是可重用性和访问解析、验证等附加功能。但**对于简单的情况**，当您只需要在一个表单中使用自定义字段或不一定需要验证时，内联字段组件就足够了👌。

## 创建自定义内联字段

假设我们为*关于我*页面设置了一个[TinaCMS表单](https://tinacms.org/docs/plugins/forms)：

> *注意：*以下示例将引用Next.js设置，但这种方法也可以应用于Gatsby。
...
注意在所有其他字段对象中，`component`属性引用的是TinaCMS字段插件，而**在我们的自定义内联字段中，我们传递的是一个React组件。**

![侧边栏中的自定义内联字段](/img/blog/custom-field-inline.png)

现在这个示例组件非常简单——一个美化的标签。这种类型的组件可以帮助组织或自定义侧边栏，但*我们可以更进一步，传递更复杂的字段*。

## 自定义范围滑块 🎨

假设我们在*关于我*页面上有一张图片，我们希望能够控制该图片的一些[CSS滤镜](https://css-tricks.com/almanac/properties/f/filter/)。下面的笔展示了我们可以使用的所有CSS滤镜。
...
要创建自定义输入字段，我们需要制作一个**接收输入并在输入更改时更新数据的React组件**。在这个例子中，我们将制作一个[范围输入字段](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/input/range)，它处理饱和度值的状态，并在范围控件滑动时更新该状态。
...
[react-final-form文档](https://final-form.org/docs/react-final-form/api/Field#3-connect-the-callbacks-to-your-input)对`input`和`meta` props进行了非常好的描述。在创建自定义字段时，您通常会访问`field`和`input` props。

#### 自定义字段应该放在哪里？

正如我们在第一个示例中看到的，我们可以通过`component`属性直接传入自定义字段组件——`component: () => <p>Hi<p>`。但当我们创建更复杂的字段时，我们很可能希望将字段提取到自己的函数中。

在上面的示例中，`RangeInput`可以在设置TinaCMS表单的`AboutMe`组件旁边定义：

```jsx
/*
** 自定义字段定义在使用TinaCMS表单的组件旁边
*/
...
它也可以在自己的文件中定义，并导入到配置TinaCMS表单选项的文件中：
...
正如开发中的许多事情一样，答案**取决于您的使用情况**😉。可以参考这个[演示仓库](https://github.com/kendallstrautman/llama-filters/blob/master/pages/Index.js)来查看Next.js的工作示例结构。

### 2. 将值添加到源数据

现在自定义输入字段已定义，我们需要将`image_saturation`值添加到我们的源数据中。源数据可以是Markdown或JSON文件。如果您已经设置了TinaCMS表单，它应该与数据源链接，因此请前往该文件。

对于我们的示例，假设我们有一个名为`about.json`的本地JSON文件。这个文件包含用于*关于我*页面的数据。在其中我们可以添加`image_saturation`值。

该值可以是存在于我们`RangeInput`组件中定义的范围内的任何整数或浮点数——0到10，步长为0.1（意味着范围的每个“滑动步”将值增加或减少0.1）。作为饱和度值，**零将是完全灰度**或无色，因此我们可以填入类似3的值以获得更“正常”的外观。

```JSON
// 示例关于我页面源数据 --> about.json
{
 “name”: “Koba Weasley”,
...
> 如果您使用的是Gatsby，您将**需要更新您的GraphQL查询**以获取这个新数据。将`image_saturation`字段添加到您的查询中。

现在我们有一个可以连接到自定义输入字段的源值。这样，**TinaCMS可以同步更新源文件中的值**，与`RangeInput`组件检测到的更改保持一致。

### 3. 将自定义字段添加到TinaCMS表单

我们来把这个自定义字段连接到TinaCMS吧？🎊
...
如果一切顺利，我们的自定义输入字段应该已经连接好了，但还有最后一件事要做。我们还没有将*饱和度值*与CSS滤镜连接起来，以便在图像上实际看到效果。
...
一个好的下一步是*为自定义`RangeInput`组件添加样式*。您可以使用[`@tinacms/styles`](https://tinacms.org/docs/plugins/fields/custom-fields#using-tina-styles)来适应其他TinaCMS字段的风格✌️。或者您可以自由发挥，以自己的方式装饰侧边栏🤠。
...
> 可以随时查看这个自定义范围输入字段的[完成版本](https://github.com/kendallstrautman/llama-filters)，或者在Tina Grande仓库中看看更复杂的[*Authors*字段插件](https://github.com/tinacms/tina-starter-grande/blob/master/src/fields/authors.js)。

### 收获 🕺🏻

为TinaCMS制作自定义字段组件非常令人兴奋！希望这篇文章能激发您的创造力，让您在内容编辑体验中尝试各种变量。
