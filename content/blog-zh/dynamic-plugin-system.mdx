...
seo:
  ...
  description: >-
    TinaCMS的动态插件系统允许您通过灵活、可重用的插件扩展功能并自定义您的内容管理体验。
title: TinaCMS中的动态插件
...
以下定义的postCreatorPlugin为例；它在侧边栏中添加了一个按钮，以便您可以创建新的博客文章：
...
在大多数系统中，我们会在启动时添加插件，并在整个会话期间保留它们。无论您在网站的哪个位置，您都可以创建一个“Post”。（注意：您可以通过让插件隐藏自身来解决这个问题，但增加的责任会使API复杂化。）

在TinaCMS中，我们可以通过编程方式添加和移除插件。这一特性为开发者提供了很大的权力——他们可以根据上下文决定如何从CMS中添加和移除功能。
...
您的网站是多语言的吗？如果有人正在浏览法语版本，让CMS将新页面添加到法语网站，而不是英语网站。

您有文章和事件吗？仅在/blog中提供创建文章的功能，而仅在/events中提供创建事件的功能。

这个插件系统展示了控制反转原则如何影响TinaCMS。决定何时启用CMS的某些功能是一项复杂的责任。静态插件系统将其交给系统本身或插件开发者。TinaCMS将这一责任交给插件用户。

这种方法对双方都有利：插件作者不必担心实现复杂的配置方案，开发者可以在简单和复杂的用例中应用插件，而无需使用黑客手段。

有兴趣创建自己的插件吗？请查看贡献[指南](https://tinacms.org/docs/contributing/guidelines)。欢迎在[TinaCMS Discord](https://discord.com/invite/zumN63Ybpf)上发布任何问题或评论。