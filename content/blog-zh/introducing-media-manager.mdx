...
到目前为止，Tina中的媒体更改是通过图像字段处理的。通过点击字段，编辑者可以从本地文件系统上传新图像。开发者会向CMS注册一个[媒体存储](/zh/docs/reference/media/overview)来处理文件上传。然而，没有办法查看已上传的图像以便在内容中使用它们。
...
在单个[`媒体`对象接口](/zh/docs/reference/media/overview)中添加了3个新属性。
...
在[媒体存储](/zh/docs/reference/media/overview)接口中添加了两个新方法。
...
另一个改进是能够在内联图像上[扩展样式](/zh/docs/contextual-editing/overview)。内联图像字段向DOM添加了一些元素，这可能会影响某些布局的样式。现在可以通过`className`或[styled-components](https://styled-components.com/docs/basics#extending-styles)直接设置样式。
...
我们统一了[`MediaStore`](/zh/docs/reference/media/overview)、[`InlineImage`](/zh/docs/contextual-editing/overview)和常规[图像字段](/docs/plugins/fields/images)之间的`previewSrc`接口。
...
现在传递的是整个[媒体对象](/zh/docs/reference/media/overview)，而不是`filename`。作为一般建议，您可以使用`media.id`（图像的完整路径）或`media.filename`来调整数据源中保存的首选最终值。
