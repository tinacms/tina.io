...
draft: false
seo:
  title: 使用Next.js创建简单Markdown博客 | TinaCMS博客
  description: >-
...
Next.js是一个基于React构建的用于开发Web应用程序的框架。具体来说，**Next.js因其引导的React环境（类似于`create-react-app`）和简单的基于文件的路由逻辑，已成为Web开发中最受欢迎的选择之一**。
...
首先，让我们**克隆起始项目**。这不过是一个用于本教程的博客应用程序模板。可以在[GitHub](https://github.com/tinalabs/nextjs-starter-boilerplate)上查看，或者使用以下命令在`my-nextjs-blog`目录中克隆起始仓库：

```
git clone https://github.com/tinalabs/nextjs-starter-boilerplate my-nextjs-blog
```

然后，进入项目文件夹，安装项目依赖，并使用以下命令启动博客应用程序：
...
![起始项目运行中](/img/blog/simple-markdown-blog-nextjs/empty-tina-blog.png "起始项目运行中")

如你所见，目前博客应用程序非常简单。现在让我们深入了解这个起始项目的结构，以**学习如何将这个应用程序转变为一个真正的基于Markdown的博客**。

## 项目结构

如果你在IDE中查看起始项目，你会看到以下文件结构：
...
这使用了**Next.js的[`Head`](https://nextjs.org/docs/api-reference/next/head)组件，使你能够指定页面头部部分的内容，以用于SEO或可访问性目的**。

需要提到的一个重要方面是，`Layout`组件使用组件级CSS。不要忘记，**Next.js开箱即用地支持[组件级CSS](https://nextjs.org/docs/basic-features/built-in-css-support#adding-component-level-css)**。这非常直观地使用。所有样式都限定在组件内。这意味着你不必担心意外覆盖其他地方的样式规则。

博客应用程序的全局样式在你可以在styles目录中找到的`globals.css`中处理。因此，如果你想更改或添加一些全局CSS规则，可以在那里进行。同时，**请记住，全局字体并没有在`global.css`文件中定义**。这是在下面的Next.js `_app.js`文件中定义的：
...
首先，在项目的根文件夹中创建一个名为`posts`的新目录。这个文件夹将包含你所有的Markdown博客文章。如果你还没有准备好内容，只需添加一些虚拟博客文章。考虑使用**[Unsplash](https://unsplash.com/)**获取示例照片，而**[Cupcake](http://www.cupcakeipsum.com/)**、**[Hipsum](https://hipsum.co/)**或**[Sagan Ipsum](http://saganipsum.com/)**可以帮助你为文章生成文本。

### 创建一个示例文章

现在你有了一个`posts`文件夹，是时候**用一些Markdown文章填充它**。

以下是`/posts/my-post.md`的示例填充内容，包含常用的前置元数据值。
...
大脑是智慧的种子，某些不可思议的事情正等待被发现。
```

如果你不熟悉这个概念，**前置元数据是一种在Markdown文件中存储元数据的方法**。通常，**前置元数据以[YAML](https://en.wikipedia.org/wiki/YAML)格式存储**，位于Markdown文件开头由三个破折号包裹的块中。
...
这对于静态页面来说很棒，但你希望有一个单一的模板，从中构建所有博客文章，从每个Markdown文件中获取不同的数据。这意味着你需要实现**动态路由**。具体来说，**你希望每个博客文章都有一个与基于此模板的页面关联的漂亮URL**。

在Next.js中，这可以非常容易地实现。具体来说，**Next.js中的[动态路由](https://nextjs.org/docs#dynamic-routing)通过文件名中的方括号`[]`标识**。在这些括号中，你可以将一个查询参数传递给页面组件。
...
让我们编写`BlogTemplate`博客页面组件，该组件将渲染从posts中读取的Markdown文件中包含的内容。通过这个页面，大部分博客逻辑将被实现。

在存储在blog目录中的`[slug].js`页面组件中，**你将能够访问通过URL传递的任何字符串，通过`slug`参数**。通常，这样的信息用于动态检索要渲染页面的数据。例如，如果你访问`http://localhost:3000/blog/julius-caesar`，则`[slug].js`中的slug查询参数将包含`"julius-caesar"`字符串。

现在让我们学习如何使用slug参数来检索你的内容数据。

### 获取博客页面组件的Markdown数据

通过动态路由，你可以**利用`slug`参数**。具体来说，你可以在`getStaticProps()`中使用`slug`从相应的Markdown文件中获取数据，如下所示：
...
当调用`getStaticProps()`函数时，该`params`对象通过[context参数](https://nextjs.org/docs/api-reference/data-fetching/get-server-side-props#context-parameter)传入。然后，从存储在`context`中的查询参数中提取`slug`。具体来说，**`slug`用于在posts目录中搜索具有相同文件名的`.md`文件**。

一旦你从该文件中获取数据，**你可以解析Markdown主体中的前置元数据并返回其数据**。这些数据作为props传递给`BlogTemplate`组件，该组件将根据需要渲染这些数据。

### 实现`getStaticPaths()`

此时，你应该对`getStaticProps()`更加熟悉。但**[`getStaticPaths()`](https://nextjs.org/docs/basic-features/data-fetching/get-static-paths)**函数可能对你来说是新的。由于此模板使用动态路由，你需要为每个博客定义一个路径列表。这样，**Next.js将能够在构建时静态渲染每个博客文章**。请记住，你只需要在涉及动态路由时使用`getStaticPaths()`。
...
这里的`getStaticProps()`函数可能看起来有些复杂，但让我们一步一步来。这里的逻辑基于Webpack提供的[`require.context()`](https://webpack.js.org/guides/dependency-management/#requirecontext)函数。这**允许你根据三个参数创建自己的Webpack上下文**：
...
借助Webpack上下文，你可以从特定目录中挑选出与正则表达式匹配的所有文件。这使你能够**从每个文件名生成slug字符串，读取其内容，使用frontmatter库解析它，并将处理后的数据作为props传递给Index**。

然后，博客数据作为属性传递给`BlogList`组件。在`BlogList`组件中，你可以迭代博客数据并根据需要渲染文章预览列表。具体来说，**`BlogList`组件负责渲染博客数据**。
