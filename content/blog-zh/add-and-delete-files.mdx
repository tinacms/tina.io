...
[创建](/blog/add-and-delete-files#creating-new-files)和[删除](/blog/add-and-delete-files#deleting-files)内容——CMS的两个基本方面。本文将介绍如何在[Gatsby](https://www.gatsbyjs.org/)网站上使用TinaCMS设置此功能。但首先，先来个概述。
...
要使网站上的内容可编辑，您需要向Tina注册一个表单。表单出现在侧边栏中，显示[字段](/docs/reference/fields)，您可以在页面上编辑内容。

插件扩展了核心CMS的功能。在幕后，插件在Tina中做了一些重要的工作。它们注册表单，创建新的屏幕视图，并允许我们添加新内容。如果您有兴趣了解更多，请阅读Tina的[动态插件系统](/blog/dynamic-plugin-system)这篇文章。
...
1. [设置一个`content-creator`插件](/blog/add-and-delete-files#step-1-set-up-a-content-creator-plugin)
2. [在Tina中注册插件](/blog/add-and-delete-files#2-register-the-plugin-with-the-sidebar)
3. [自定义`create-form`](/blog/add-and-delete-files#3-customize-the-create-form)
4. [为新文件配置默认数据](/blog/add-and-delete-files#4-configure-defaults)
