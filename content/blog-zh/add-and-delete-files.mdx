...
consumes:
  ...
  - file: /packages/tinacms/src/components/CreateContent.tsx
    details: 包含创建表单外观的图片
warningMessage: >-
  **更新：** 本文中的示例引用了过时的Gatsby实现。我们建议使用[Next.js](/zh/docs/setup-overview/)以获得更流畅的解决方案。
seo:
...
[创建](/zh/blog/add-and-delete-files#creating-new-files)和[删除](/zh/blog/add-and-delete-files#deleting-files)内容——CMS的两个基本方面。本文将介绍如何在[Gatsby](https://www.gatsbyjs.org/)网站上使用TinaCMS设置此功能。但首先，先来个概述。
...
要使网站上的内容可编辑，您需要向Tina注册一个表单。表单出现在侧边栏中，显示[字段](/zh/docs/reference/fields)，您可以在页面上编辑内容。

插件扩展了核心CMS的功能。在幕后，插件在Tina中做了一些重要的工作。它们注册表单，创建新的屏幕视图，并允许我们添加新内容。如果您有兴趣了解更多，请阅读Tina的[动态插件系统](/zh/blog/dynamic-plugin-system)这篇文章。
...
1. [设置一个`content-creator`插件](/zh/blog/add-and-delete-files#step-1-set-up-a-content-creator-plugin)
2. [在Tina中注册插件](/zh/blog/add-and-delete-files#2-register-the-plugin-with-the-sidebar)
3. [自定义`create-form`](/zh/blog/add-and-delete-files#3-customize-the-create-form)
4. [为新文件配置默认数据](/zh/blog/add-and-delete-files#4-configure-defaults)
...
有不同的插件用于创建新的markdown或JSON文件。在本教程中，我们将使用`RemarkCreatorPlugin`来处理markdown文件。请随意阅读[文档](/zh/docs/guides/converting-gatsby-to-tina)中的`JsonCreatorPlugin`。
...
您可以使用`create-form`的数据来创造性地生成新文件路径或填充默认数据（稍后会详细介绍）。总体而言，格式化`filename`取决于您的项目结构。选择一个对您有意义的简单解决方案，或者在[这里](/zh/docs/reference/self-hosted/git-provider/github)查看更多示例。
...
前往您在项目中配置了Tina表单的文件。这通常是生成多个帖子、案例研究等的模板文件。如果您的项目中没有配置Tina表单，请在[这里](/zh/docs/guides/converting-gatsby-to-tina)了解有关使用Gatsby+Tina创建表单的更多信息。
