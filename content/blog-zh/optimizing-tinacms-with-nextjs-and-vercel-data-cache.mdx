...
next: ''
---
**更新于2025年3月31日：** 添加了一些图片，以提供有关启用Vercel数据缓存和可能问题的更多信息。

**更新于2025年3月19日：** 添加了一个视频，演示Vercel数据缓存如何影响您的网站以及如何调整Next.js以更好地使用缓存。

<Youtube embedSrc="https://youtube.com/embed/gz9Lxlj5RDU" caption="TinaCMS - 使用nextjs和vercel数据缓存" minutes="6" />

在2024年12月，随着`tinacms@2.5.2`和`@tinacms/cli@1.7.0`的发布，TinaCMS过渡到使用Node.js的原生fetch函数，消除了对fetch-ponyfill依赖的需求。这一变化减少了依赖项，提高了安全性和性能。它还使我们能够支持**Vercel数据缓存**。
...
1. **调整重新验证时间**\
   修改路由段的`revalidate`设置，以控制Vercel检查更新的频率。这使得增量静态再生（ISR）能够用新数据刷新页面。更多详情请参阅[路由段配置 - revalidate](https://nextjs.org/docs/app/api-reference/file-conventions/route-segment-config#revalidate)。
2. **使用查询参数进行新鲜抓取**\
   在您的TinaCMS GraphQL查询中添加一个`revalidate`参数，确保Next.js抓取请求在设定的时间间隔内更新：
   `const response = await client.queries.page(`
   `   { relativePath: 'pages/home.mdx' },`
   `   { fetchOptions: { next: { revalidate: 60 } } }`
   `);`
   这种方法确保数据每60秒重新验证一次。
   更多详情请参阅[Next.js抓取扩展](https://nextjs.org/docs/app/api-reference/functions/fetch)。

还值得注意的是，在**Next.js 14**中，诸如fetch()和GET路由处理程序等缓存机制默认自动启用，开发人员需要在需要动态行为时选择退出。相比之下，**Next.js 15**引入了一个重大变化，不再默认缓存这些机制，使开发人员能够根据需要选择加入，从而更明确地控制缓存策略。
...
您可以在TinaCMS文档中找到更多信息 [tina.io/docs/frameworks/next/overview](/zh/docs/frameworks/next/overview#updates-not-reflecting-correctly-on-page)
