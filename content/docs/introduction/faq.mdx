...
By default when using TinaCloud, the author on Git commits is the [Tina Cloud GitHub app](https://github.com/apps/tina-cloud-app). Some users prefer to know who made the change.

For this use case, it is possible to [co-author](https://docs.github.com/en/pull-requests/committing-changes-to-your-project/creating-and-editing-commits/creating-a-commit-with-multiple-authors) Git commits in order to attribute the commit to an additional user. This can be enabled per-user in the Tina Cloud account settings.
