...
There is a known limitation where Tinacms doesn't load assets correctly when the admin is deployed to a sub-path: (`https://jamespohalloran.github.io/my-site-root/admin/` for example). To resolve the issue, in your [Tina config file](/docs/reference/config/ "Tina config file"), set the `build.basePath` property to value of the sub-path (e.g. `my-site-root`).
...
xxx - this should not be rendered
