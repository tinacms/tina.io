...
last_edited: '2025-03-17T03:03:54.810Z'
next: ''
previous: ''
---
...
A less ideal solution would be to pass the outer object housing the field we want from Tina. This allows us to cherry pick the field and use it for our `[data-tina-field]` attribute. While this approach works well for fields with multiple layers of nesting, it can make it difficult to figure out which elements on our page we can edit contextually from a glance.
...
This approach is a little tricker to get right in a type safe manner. Using a forwardRef instead of a component we can apply the `[data-tina-field]` attribute to the React component being edited. The attribute gets passed in directly when we use the spread operator on `props`.
...
* The \[data-tina-field] helper does not affect the flow of data from Tina. It simply determines which part of the form Tina will open when the user clicks a given HTML element.
* `[data-tina-field]` cannot be applied directly to the [TinaMarkdown](https://tina.io/docs/editing/markdown#rendering-content-with-tinamarkdown) component. This is because the TinaMarkdown component doesn't render a root element in the DOM.
