...
TinaCMS's "click to edit" feature allows editors to select the **HTML element** they want to click on the page in order to see the corresponding field in the sidebar.
...
When using `TinaMarkdown` with custom components, the `props` object passed into each component already includes the `_content_source` metadata. This metadata is **automatically injected in edit mode** via TinaCMS's `useTina` hook.
...
This gives TinaCMS a more precise mapping to the field, based on the template definition defined in your collection.
...
When TinaCMS is in edit mode, it enhances the data with metadata for each editable field. For example, a rich text template field like  might look like:
...
When edit mode is active, TinaCMS automatically applies outlines to any `[data-tina-field]`element.
