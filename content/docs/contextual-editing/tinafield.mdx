...
last_edited: '2025-03-17T01:06:54.378Z'
next: ''
previous: ''
---
...
* The \[data-tina-field] helper does not affect the flow of data from tina, it simply determines which Tina field will open when the user clicks an element. For example, I may use a TinaMarkdown element
...
The easiest way to assign `[data-tina-field]` to a React component is to create a custom attribute. This will allow you to to pass the output from the `tinaField` helper to the expecting HTML element directly.
...
### Passing the outer object of the field

A less ideal solution would be to pass the object housing the field we want from Tina. This allows us to cherry pick the field we want from the object. While this approach works well for fields with multiple layers of nesting, it can make it difficult to figure out which element we can edit contextually from a glance.
...
This is a little tricker to do due to the typing but by using forwardRef we can apply the `[data-tina-field]` attribute directly to the expecting React component.
