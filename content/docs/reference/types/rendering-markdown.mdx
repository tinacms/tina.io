...
last_edited: '2025-07-31T15:06:31.979Z'
next: ''
previous: ''
---
...
import MermaidElement from "@/components/mermaid-element";
...
function CodeBlock({ value, lang = "ts" }) {
  const [html, setHtml] = useState("");

  useEffect(() => {
    shikiSingleton.codeToHtml(value, lang).then(setHtml);
  }, [value, lang]);
