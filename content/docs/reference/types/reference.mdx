...
last_edited: '2025-03-14T04:36:04.696Z'
next: ''
previous: ''
---
...
> The collections property controls the set of collections we can reference.
...
This is just a guide. The generic [Field](/docs/reference/fields) type definition contains usage information.
...
When querying for a `post` document via the client or raw GraphQL, the `author` key in the response will contain the values of the _referenced_ `author` document:
...
<WarningCallout body="If you require strict typing in your application, you'll need to manually declare the type of props for `optionComponent` ahead of time. See examples below." />
...
<WarningCallout body="If you pass in a function, it should be passed as an executable. Running something like `const url = new URL('https://bob-northwind-sydney.com')` directly in the callback isn't allowed." />
