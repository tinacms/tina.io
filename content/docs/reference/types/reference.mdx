...
<WarningCallout
  body={<>
    The current `collectionFilter`only supports properties of type string and is also **case sensitive** ( e.g. Using the example below, if you enter `australia`, it will not filter authors by `Australia`).
  </>}
/>
