...
last_edited: '2024-09-26T06:48:42.467Z'
...
This can be used to manage repeated data structures, group related data in the CMS and/or give users the option to choose between a set of templates.
...
* with `list` set to `false`, the `fields` array can be used to wrap some data – this appears in the editor as a nested page. Such as grouping options together, as in the gif below.

![](https://res.cloudinary.com/forestry-demo/image/upload/v1727331124/tina-io/docs/Sep-26-2024_16-05-37_d2ol8w.gif)

* with `list` set to `true`, the `fields` array contains the data structure to be repeated – this appears in the editor as above, but grouped as a list. Complex lists can be created, such as lists with multiple fields in each item.

![](https://res.cloudinary.com/forestry-demo/image/upload/v1727331115/tina-io/docs/Sep-26-2024_16-03-00_f3ogev.gif)

* with `list` set to `true`, and using `templates` as opposed to `fields` the user can select one of the associated templates – this appears as above, but pressing the `+`  gives you the option to choose between templates.
