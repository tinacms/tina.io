...
interface ColorConfig {
  ...
  width?: string // Defaults to full page width
}
```
...
| `colorFormat` | Specify whether you want the color value to be a hexadecimal ('hex') or RBG value. *(Optional)*                                                                                                                                              |
| `colors`      | An array of 'swatch' values that will either display as options below the "sketch" widget, or will serve as swatch options for the "block" widget. If no colors are passed, a set of default colors will render, ROYGBIV style. *(Optional)* |
| `widget`      | An optional string indicating whether the "sketch" or "block" widget should render. This will default to "sketch" if no value is passed. *(Optional)*                                                                                        |
| `width`       | An optional string to set the width of the color picker. This will default to full page width if no value is passed. *(Optional)*                                                                                                            |
