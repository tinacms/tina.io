...
<apiReference
  ...
  property={[
    ...
    },
    {
      ...
      description:
        "Origins allowed to make cross-origin requests to the dev server (`tinacms dev`). Has no effect on production.\n\nDefaults to `localhost` / `127.0.0.1` / `[::1]` only. Each entry can be:\n\n- An **exact origin string** \u2014 e.g. `'https://my-codespace.github.dev'`\n- A **RegExp** \u2014 e.g. `/^\\.preview\\.app$/`\n- `'private'` \u2014 expands to all RFC 1918 private-network IP ranges (`10.x`, `172.16\u201331.x`, `192.168.x`), useful for Docker, WSL2, etc.\n\n```ts\nserver: {\n  allowedOrigins: ['private', 'https://my-codespace.github.dev']\n}\n```\n"
    },
    {
      groupName: "",
      name: "branch",
    ...
    },
    {
      ...
      type: "number",
      description:
        "Minimum similarity score (0\u20131) for a fuzzy match to be included.\n",
      default: "0.6"
    },
    {
      groupName: "search.tina",
      ...
      type: "number",
      description:
        "Maximum number of fuzzy term expansions to return per search term.\n",
      default: "10"
    },
    {
      groupName: "search.tina",
    ...
    },
    {
      ...
      description:
        "Controls telemetry collection for TinaCMS.\n\nSet to `'disabled'` to opt out of anonymous usage telemetry.\n",
      default: "'anonymous'"
    }
  ]}
/>
