---
id: /docs/reference/config/
title: Config Reference
alias: the-config-file
last_edited: '2025-12-09T23:01:02.997Z'
tocIsHidden: false
next: content/docs/reference/schema.mdx
previous: content/docs/reference/overview.mdx
---

The result of the `defineConfig` function when exported from `tina/config.{ts,tsx,js,jsx}` defines your TinaCMS setup.

<WarningCallout
  body={<>
    Important - Config must be deterministic
    
    The object returned by defineConfig must be evaluated multiple times during development and build. Avoid using non-deterministic values (e.g Date.now(), Math.random()) directly in config properties that affect schema or runtime configuration. This can cause GraphQL schema mismatch errors
    
    If you need dynamic values, return a function and compute the value lazily.
  </>}
/>

Note that it **must be** the default export.

> The location for the config file was previously at `.tina/config.{ts,tsx,js,jsx}`

## Options

<apiReference
  title=""
  property={[
    {
      groupName: "",
      name: "schema",
      type: "Schema",
      description: "Defines your **content model**.\n",
      required: true
    },
    {
      groupName: "build",
      name: "publicFolder",
      type: "string",
      required: true,
      description: 'Application asset folder path.\n\nTypically, `"public"`.\n'
    },
    {
      groupName: "build",
      name: "outputFolder",
      type: "string",
      required: true,
      description:
        'The TinaCMS build path (editor route).\n\nTypically, `"admin"`.\n\nThis would be viewable at `[url]/admin/index.html`.\n'
    },
    {
      groupName: "build",
      name: "host",
      type: "string | boolean",
      description: "The host option for the vite config.\n"
    },
    {
      groupName: "build",
      name: "basePath",
      type: "string",
      description:
        'Set if serving at a subpath.\n\nFor `[url]/my-site`, set to `"my-site"`.\n'
    },
    {
      groupName: "server",
      name: "allowedOrigins",
      type: "(string | RegExp | 'private')[]",
      description:
        "Controls which websites are allowed to talk to your local dev server. Only affects `tinacms dev` — has no effect in production.\n\nBy default, only requests from your own machine (`localhost`) are allowed. You only need to set this if you develop in a remote environment like **GitHub Codespaces**, **Gitpod**, **Docker**, or **WSL2**.\n\nEach entry can be:\n\n- A **URL string** — e.g. `'https://my-codespace.github.dev'`\n- A **RegExp** — to match a pattern of URLs\n- `'private'` — a shorthand that allows any device on your local network, i.e. RFC 1918 private IP ranges (useful for Docker or WSL2)\n\n```ts\nserver: {\n  allowedOrigins: ['private', 'https://my-codespace.github.dev']\n}\n```\n"
    },
    {
      groupName: "",
      name: "branch",
      type: "string | null",
      description:
        "The base branch to pull content from. Required for **TinaCloud**.\n"
    },
    {
      groupName: "",
      name: "clientId",
      type: "string | null",
      description: "The`clientId` from TinaCloud. Required for **TinaCloud.**\n"
    },
    {
      groupName: "",
      name: "token",
      type: "string | null",
      description:
        "Your read only token from TinaCloud. Required for **TinaCloud.**\n"
    },
    {
      groupName: "",
      name: "localContentPath",
      type: "string",
      description:
        "The path to a separate content repo for local development, **resolved relative to the `tina/` config folder** (not the project root).\n\nWhen set, collection `path` values are resolved relative to this content root. The content directory **must contain a `tina/` folder** (it can be empty) — TinaCMS writes generated files like `tina-lock.json` there.\n\nUse an environment variable since the path may differ per developer:\n\n```ts\nlocalContentPath: process.env.TINA_CONTENT_REPO_PATH // e.g. '../../my-content-repo'\n```\n\nSee the [Separate Content Repo guide](/docs/guides/separate-content-repo) for full setup details.\n"
    },
    {
      groupName: "",
      name: "authProvider",
      type: "AuthProvider",
      description:
        "Set-up for [custom authentication](/docs/reference/self-hosted/auth-provider/overview).\n"
    },
    {
      groupName: "admin",
      name: "authHooks",
      type: "AuthHooks",
      description: "Used to run logic on authentication events.\n"
    },
    {
      description:
        "Opt out of update checks - this will prevent the CMS for checking for new versions. If true, the CMS will not check for updates.\n",
      groupName: "ui",
      name: "optOutOfUpdateCheck",
      type: "boolean",
      default: "false"
    },
    {
      groupName: "ui",
      name: "previewUrl",
      type: "function -- context: { branch: string } => { url: string }",
      description:
        "Identify branch-specific URL patterns, such as for preview deployments, and link to them via editor.\n\n![](/img/docs/share-btn_xvmxii.png)\n",
      required: false
    },
    {
      description:
        'A regular expression pattern that folder names must match when creating new folders. Only applies to newly created folders, not existing ones.\n\nEg: "^\\[a-z0-9-]+$" for lowercase letters, numbers and hyphens\n',
      groupName: "ui",
      name: "regexValidation.folderNameRegex",
      type: "string"
    },
    {
      groupName: "client",
      name: "skip",
      type: "boolean",
      description: "Skip building the autogenerated GraphQL client.\n"
    },
    {
      groupName: "client",
      name: "referenceDepth",
      type: "number",
      description:
        "Generated queries will traverse references to a given depth.\n",
      default: "2"
    },
    {
      groupName: "client",
      name: "errorPolicy",
      type: "'throw' | 'include'",
      description:
        "Sets client error handling. 'throw' will throw the error on query fail, 'include' will return it in the query response.\n",
      default: "throw"
    },
    {
      groupName: "media",
      name: "loadCustomStore",
      type: "() => Promise<Store>",
      description:
        "Load a custom media store like Cloudinary.\n\n**Either set this property or the** `media.tina` **object, not both.**\n"
    },
    {
      groupName: "media",
      name: "accept",
      type: "string | string[]",
      description:
        'Allowed media types.\n\nFor example `["image/png", "image/jpeg"]` or `"image/*"`.\n'
    },
    {
      groupName: "media.tina",
      name: "publicFolder",
      type: "string",
      description: "Your asset file-path.\n"
    },
    {
      groupName: "media.tina",
      name: "mediaRoot",
      description:
        "The root folder for media (subdirectory of the public folder).\n",
      type: "string"
    },
    {
      groupName: "media.tina",
      name: "static",
      description: "Indicates whether editors can manage media.\n",
      default: "false",
      type: "boolean"
    },
    {
      groupName: "search",
      name: "indexBatchSize",
      type: "number",
      description: "The number of documents to index per PUT request.\n"
    },
    {
      groupName: "search",
      name: "maxSearchIndexFieldLength",
      type: "number",
      description:
        "The maximum length of a string field that will be indexed for search.\n"
    },
    {
      groupName: "search",
      name: "searchClient",
      type: "SearchClient",
      description:
        "An instance of a custom search client like Algolia.\n\n**Either set this property or the** `search.tina` **object, not both.**\n"
    },
    {
      groupName: "search.tina",
      name: "indexerToken",
      type: "string",
      description:
        "Search index token with permissions to write to search index. Only used by CLI.\n"
    },
    {
      groupName: "search.tina",
      name: "stopwordLanguages",
      type: "string[]",
      description: "Stopword languages to use.\n",
      default: '["eng"]'
    },
    {
      groupName: "search.tina",
      name: "tokenSplitRegex",
      type: "string",
      description: "Regex used for splitting tokens.\n",
      default: "/[\\p{L}\\d_]+/"
    },
    {
      groupName: "search.tina",
      name: "fuzzyEnabled",
      type: "boolean",
      description: "When enabled, search will return results even if the query has minor typos or spelling differences.\n",
      default: "true"
    },
    {
      groupName: "search.tina",
      name: "fuzzyOptions.maxDistance",
      type: "number",
      description: "How many character changes (insertions, deletions, substitutions) a word can be from the search term and still count as a match. Higher values return more results but may be less relevant.\n",
      default: "2"
    },
    {
      groupName: "search.tina",
      name: "fuzzyOptions.minSimilarity",
      type: "number",
      description:
        "A threshold between 0 and 1 for how closely a result must resemble the search term. Raise this to only show very close matches; lower it to be more lenient.\n",
      default: "0.6"
    },
    {
      groupName: "search.tina",
      name: "fuzzyOptions.maxTermExpansions",
      type: "number",
      description:
        "The maximum number of similar words the search engine will consider for each search term. Higher values can improve recall but may slow down search.\n",
      default: "10"
    },
    {
      groupName: "search.tina",
      name: "fuzzyOptions.useTranspositions",
      type: "boolean",
      description:
        "When enabled, swapped characters (e.g. typing \"teh\" instead of \"the\") count as a single edit rather than two, making the search more forgiving of common typos.\n",
      default: "true"
    },
    {
      description:
        "The defaultBranchName is used for overriding the local branch when resolving the `historyUrl` on local. The primary use case is for when the default branch is not called `main`\n",
      groupName: "repoProvider",
      name: "defaultBranchName",
      type: "string"
    },
    {
      description:
        "Allows users to define where TinaCMS should link users to view the source history of the file they are editing.\n\n`branch` is resolved by TinaCMS for the active editing branch\n\n`relativePath` is resolved by TinaCMS for the relative path of the relevant content file\n\nExample for GitHub:\n\n```\nhistoryUrl: ({ relativePath, branch }) => ({ \n  url: `https://github.com/tinacms/tina.io/commits/${branch}/${relativePath}`\n```\n",
      groupName: "repoProvider",
      name: "historyUrl",
      type: "function -- historyUrl?: (context: { relativePath: string; branch: string;}) => { url: string }"
    },
    {
      groupName: "",
      name: "contentApiUrlOverride",
      type: "string",
      description: "Override the default content API URL.\n",
      experimental: true
    },
    {
      groupName: "",
      name: "telemetry",
      type: "'anonymous' | 'disabled'",
      description:
        "Controls whether TinaCMS collects anonymous usage statistics to help improve the product.\n\nSet to `'disabled'` to opt out entirely.\n",
      default: "'anonymous'"
    }
  ]}
/>

## Examples

### Vercel and TinaCloud Configuration

This is a standard configuration using TinaCloud to host your datalayer and the built-in Tina media and search capabilities.

```ts
const branch =
  process.env.NEXT_PUBLIC_TINA_BRANCH ||
  process.env.NEXT_PUBLIC_VERCEL_GIT_COMMIT_REF ||
  process.env.HEAD ||
  'main';

export default defineConfig({
  branch,
  // generated by TinaCloud
  token: '<Your Read Only Token>',
  // generated by TinaCloud
  clientId: '<Your Client ID>',
  build: {
    publicFolder: 'public',
    outputFolder: 'admin',
  },
  schema: {
    //Content model definition here...
  },
  ui: {
    previewUrl: (context) => {
      // Use Vercel preview deployments based on branch names
      return { url: `https://my-app-git-${context.branch}.vercel.app` };
    },
  },
  media: {
    tina: {
      publicFolder: 'public',
      mediaRoot: 'uploads',
    },
  },
  search: {
    tina: {
      indexerToken: process.env.TINA_SEARCH_TOKEN,
    },
  },
});
```

### Cloudinary Media Store

Tina supports both Git-backed media and specific external media providers:

```ts
export default defineConfig({
  // ... other config options
  media: {
    loadCustomStore: async () => {
      const pack = await import('next-tinacms-cloudinary');
      return pack.TinaCloudCloudinaryMediaStore;
    },
    accept: 'image/*',
  },
});
```

### TinaCloud Search Options

Tina provides built-in search capabilities through TinaCloud.

```ts
// Tina Cloud search
export default defineConfig({
  // ... other config options
  search: {
    tina: {
      indexerToken: process.env.TINA_SEARCH_TOKEN,
      stopwordLanguages: ['eng', 'fra'],
      tokenSplitRegex: '/[p{L}d_]+/',
    },
  },
});
```

### Imports with Typescript Path Aliases

TinaCMS supports TypeScript path aliases in the `tina/config.{ts,js,tsx}` file and collections.

Ensure that `tsconfig.json` exists at the root of your project. Define your aliases in the `compilerOptions` section.

```javascript
//tsconfig.json
{
  "compilerOptions": {
    "baseUrl": ".",
    "paths": {
      "@/*": ["src/*"]
    }
  }
}
```

You can now import utilities or other modules using the aliases.

```typescript
import { someUtility } from '@/lib/utils';

export default defineConfig({
  //...
});
```
