...
id: fields
title: Fields
last_edited: '2024-09-12T05:46:43.912Z'
...
`ui.max` only takes effect on `object` field types with `list: true` specified. It also isn't compatible with `object` field types using `templates`. It disables the add button in the editor once a certain number of elements have been added.
...
`ui.min` can apply to all field types with list `list: true` specified, provided they appear as a list in the editor. Specifically, this is the:
...
![](https://res.cloudinary.com/forestry-demo/image/upload/v1726119937/tina-io/docs/Screenshot_2024-09-12_at_3.45.11_pm_newuzo.png)
