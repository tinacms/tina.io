...
<apiReference
  ...
  property={[
    ...
    {
      description:
        "The max line width used when serializing frontmatter for markdown and for content in YAML files. This sets the line length that reformats long strings with folded scalar syntax (>-). Setting to `-1` disables line wrapping.\n",
      name: "yamlMaxLineWidth",
      type: "number?",
      default: "-1"
    },
    {
    ...
    },
    {
      ...
      description:
        "A function called before the form is submitted. Any return value will replace the form submit value. To query content inside this function use the `cms` argument. See [Before Submit Function](/docs/extending-tina/before-submit).\n"
    },
    {
      groupName: "ui",
      name: "global",
      type: "boolean",
      description: "Marks the collection as global: a single, site-wide settings document edited from the Site area of the sidebar rather than the collection list. See [Global collections](#global-collections).\n",
      default: "false"
    },
    {
      groupName: "ui",
...
<WarningCallout
  body={<>
    In **Local Mode**, when **`path`** is changed, you will need to run `tinacms dev`. This ensures the generated files include the new path.
  </>}
/>
...
* It appears once under the **Site** heading in the sidebar (with a globe icon), instead of in the main collections list.
* Opening it goes straight to the document's form — there's no document list to step through, because a global collection is meant to contain a single document.

![Global collections (Global, SEO Defaults, Announcement Bar) listed under the Site heading in the TinaCMS sidebar, each with a globe icon, separate from the regular Collections list above.](/img/docs/reference/global-collections.png)
...
<WarningCallout
  body={<>
    A global collection is intended to hold a single document. If it contains zero or multiple documents, the editor falls back to the regular list view.
  </>}
/>
