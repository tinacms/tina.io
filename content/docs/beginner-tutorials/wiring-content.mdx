...
* We are introducing the `useTina` method, which is how TinaCMS "wires up" the content we see on the page with the inputs in visual editor. `useTina` takes an object that includes a few properties from our GraphQL response, so TinaCMS's visual editor can populate its inputs with the same values.
