...
last_edited: '2025-03-18T06:57:41.136Z'
tocIsHidden: true
next: content/docs/beginner-tutorials/wiring-content.mdx
previous: content/docs/beginner-tutorials/display-content.mdx
---

Right now, if we want to edit our amazing title, we have to go into the Tina editor, find our collection, and then edit the record in our collection before we can view our changes. Wouldn't it be cool if we could make edits to the title, and see our changes directly on the page while we're editing our content? Let's do that now!
...
Secondly, we are cheating a little here by hard-coding the `"Hello-World"` filename directly to the home route. We can make this better later, but for now let's just roll with it.
