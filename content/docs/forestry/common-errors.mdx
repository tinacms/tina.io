...
The concept of using blocks as field types (previously available in Forestry) is supported in TinaCMS as [providing templates in an object field](/docs/editing/blocks/). During the migration, the migration tool will convert your blocks into templates.
...
Then in TinaCMS, your block field would look like:
...
..where "my\_template" is the name of the template in the collection to which the file belongs.
