...
## 3. Do I need a certified person to use TinaCMS?

Anyone can start using TinaCMS right away but it is designed for developers to setup and customize. A [certified developer](https://www.credly.com/org/superior-software-for-windows-ssw/badge/tinacms-developer) can accelerate the setup, fine-tune the configuration, and ensure best-practice architecture.
...
## 5. What is TinaCMS's tech stack?
...
## 6. What features are unavailable in self-hosted TinaCMS compared to TinaCloud?

When comparing TinaCloud with self-hosting TinaCMS, there are specific features unique to TinaCloud. These include:
...
See the TinaCloud [troubleshooting](/docs/tinacloud/troubleshooting) page for more information.
...
## 17. Is TinaCMS LLM-friendly?

Yes. TinaCMS is designed to be highly LLM-friendly, mainly because it uses Markdown for structuring content.
...
## 19. Do I need a GitHub account to edit?

No! Only the individual who sets up the TinaCloud project, linking it to a GitHub repository, requires GitHub.
...
## 20. GitHub Copilot branches not indexed by TinaCloud

Due to a current limitation in GitHub Copilot’s webhook behaviour, branches created through GitHub Copilot assignments are not automatically indexed by TinaCloud. These branches do not trigger the branch creation webhook that TinaCloud relies on to begin indexing content.
