...
<Youtube embedSrc="https://youtube.com/embed/4fqTYtfpbBk" /> **Video: TinaCloud - How to index missing branches (3 min)**

Option B:
You can also force a branch to be reindexed by TinaCloud by making a whitespace change to the `tina/tina-lock.json` file in that branch, committing it, and pushing it to GitHub. This will initiate indexing for the branch, and (after a few minutes) the error should be resolved
