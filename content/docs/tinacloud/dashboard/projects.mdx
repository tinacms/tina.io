...
This authorization will allow TinaCloud to push and pull content to and from the GitHub repository associated with your site.
...
> If your project uses [repo-based media](/docs/reference/media/repo-based/), TinaCloud will automatically trigger an initial media sync when the project is created. You can monitor the sync status and manually re-sync at any time from the **Media** tab in your project dashboard.
...
From the project configuration screen, click the "Change Repository" button. You will authorize Tina.io again and a list of repositories will be displayed. Select the new repository and click Save Project to update the repository.
...
If your `tina` directory is at the project root (as is for most sites), this field can be left blank.

If your GitHub repository uses a monorepo structure, the Path To Tina Config input can be used to specify the path to the
`tina` directory in your repository.

For example, if you have a project named `northwind-docs`, and it is located in the
`projects` directory of your repository, the Path To Tina Field would be `projects/northwind-docs` .

Example setup for Path to Tina Field = projects/northwind-docs
...
![](/docs/path-to-tina-folder.png "Path to Tina Folder UI under Advanced Settings")
...
<WarningCallout
  body={<>
    Warning: API tokens should be considered sensitive and not be committed to your project repository. Content tokens allow clients read-only access to your repository content. Search tokens allow clients to update the TinaCloud hosted search index for your site. Please use environment variables for these in your Tina configuration.
  </>}
/>
...
![TinaCloud token tab](/img/docs/tinacloud/api_tokens_list_v1_mmjpsp.png)

Next, click "New Token" and fill out the fields. The "Type" field selects the type of token being created. "Name" is used for identifying the token. "Git Branches" is the list of branches (separated by commas) that the token has access to.

![Creating a new token in TinaCloud](/img/docs/tinacloud/new_api_token_v1_lq1rfy.png)
