...
last_edited: '2025-11-25T22:25:42.313Z'
...
> ⚠️ **Simple option**: Keeps Vercel's auto-deploy, but conditionally ignores builds

> ⚠️ **Advanced option**: Completely disables Vercel's auto-deploy, full control via GitHub Actions

> ⚠️ **Important limitation of both options**: Due to the way Vercel is configured internally, every commit to a tracked branch will still count towards your quotas and concurrent build count

> ⚠️ **It is not recommended to use both options at the same time. It can lead to race conditions or undesirable behaviour. If you can handle the complexity, the advanced option is the recommended choice. It is a much more flexible and scalable solution for most custom use cases.**

### Simple option

Use Vercel's built-in functionality - follow the [Ignore Build Step Guide](https://vercel.com/docs/project-configuration/git-settings#ignored-build-step) to define a Bash or Node script to conditionally reject deploying certain builds.

Set your **`Ignore Build Step`** to **`Run my Bash script`** or **`Run my Node script`**. Put your script in your repo and supply a path to it to the command, e.g. **`bash my-deployment-conditions.sh`**. Your file will be similar to the one below, which only deploys the production version.

<ImageAndText
  docText={<>

  </>}
  heading="Vercel Bash Setting"
  image="/img/docs/tina-cloud-deployment-options/Vercel%20Bash.png"
/>
...
In Vercel, navigate to **My Account | Settings | Tokens**, create a token with a **`Scope`** of your projects and an appropriate expiration. Give it a descriptive name. **Copy the token when prompted to, you will need it for the next step, and you won't be able to see it again.**

In Vercel, navigate to your project **Settings | Project ID** and copy the **`Project ID`** from there.

Navigate to your project and copy the site URL, e.g. [https://mycoolblog.vercel.app/](https://mycoolblog.vercel.app/).

Navigate to your Vercel account **Settings | General | User ID** and copy **`User ID`** from there. That will be used as an organisation ID at a later step.
...
> ⚠️ Variable names must match exactly for the Workflow to work.
