...
seo:
  description: Learn how to manually control TinaCMS's edit state using the useEditState hook, enabling custom edit mode toggling functionality
---

## Manually toggling via `useEditState`

You can manually enter and exit edit mode by tapping into the `useEditState` hook. You should **always** have a [`/pages/admin.{js,tsx}` file](/docs/contextual-editing/overview) but in some cases you may want to go into edit mode without going to the `/admin` or `/admin#/logout` routes.
