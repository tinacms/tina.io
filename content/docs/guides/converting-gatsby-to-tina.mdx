...
last_edited: '2024-11-10T23:45:31.259Z'
...
<WarningCallout body="Warning - If you do decide to add visual editing you will need to swap out your gatsby-source plugins" />
...
Add the following import to the top of gatsby-node.ts

```diff
+ import client from "./tina/__generated__/client"
```

We'll also create new types for the response from Tina's GraphQL API and remove the existing ones.
...
We'll also update the page query to exclude the markdown from the query since we'll be using Tina to update the query instead
...
Remove the existing mdx query to prevent confusion

Update the page using the useTina hook, this will give us visual editing
...
Then we'll update the date inside of useEffect. The benefit of this approach is that we'll be able to see the date update in real time when we use the visal editor.
...
We'll also swap out the value using useState.
...
We also need to update the pages on the home page because it was being populated using gatsby-mdx before. Add the following changes to `src/pages/index.tsx`
...
similar to the home page we'll add
...
> Note: we don't need to use the useTina hook here because the home page is static
...
The final thing we'll need to do to add contextual editing is to configure the routing property of our collection. This will navigate us to the correct page
when we open the file in TinaCMS' visual editor. Because we're storing each blog post in a single folder in `content` we can just use the first
folder in the breadcrumbs array to retrieve the correct path.
