...
> Note: You may need to update other elements on your site. For unsupported markdown elements in TinaCMS, refer to [our guide](https://tina.io/docs/reference/types/rich-text/?#unsupported-elements).
...
Because we'll be using TinaCMS's graphql client for this approach we no longer need to skip it.
...
Using these types, we'll add a helper to map out the response from TinaCMS's GraphQL API.
