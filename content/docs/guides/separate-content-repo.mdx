...
last_edited: '2026-03-19T00:08:03.875Z'
next: ''
previous: ''
---
...
Tina config in your "website repo", but documents will be saved externally in a “content repo”.
...
We'll start by first creating the “website repo”. To help with this, we've created a [basic template](https://github.com/tinacms/separate-website-repo) that you can clone down and follow along with. The template contains a NextJS site, and the `tina` config with the local config path already configured.
...
Create a `.env` file in the “website repo” and fill it out with the tokens retrieved in the previous step.
...
Now that the “website repo” has been created in TinaCloud, grab the client ID retrieved in the previous step and create a new `tina/meta.json` file in the “content repo”. This is what connects the two together.
