...
You'll likely want to query the TinaCMS's Content API for [dynamic routes](https://nextjs.org/docs/basic-features/data-fetching/get-static-paths#getstaticpaths).
...
In Next.js one can specify [`fallback: "blocking"`](https://nextjs.org/docs/api-reference/data-fetching/get-static-paths#fallback-blocking), this allows `getStaticProps` to run server-side at request time when a user goes to a page that was not specified in `getStaticPaths`. This allows document-creation to work with TinaCMS, as well as advanced NextJS features like ISR.

For a full working example of TinaCMS + NextJS, [check out our "React Starter"](https://github.com/tinacms/tina-react-starter).
