...
last_edited: '2025-12-08T22:07:15.369Z'
next: ''
previous: ''
---
...
query postConnection {  postConnection {    totalCount    edges {      cursor      node {        ... on Document {          \_sys { path }        }        lastChecked      }    }    pageInfo {      hasNextPage      endCursor    }  }}
...
Runs map-links-to-content to read file contents into a JSON array of { path, content }.
...
TINA\_AUDITOR\_QUERY="query postConnection { ... }"
...
“Resource not accessible by integration”: ensure the workflow runs in the base repo (not a fork), uses token: ${{ secrets.GITHUB\_TOKEN }}, and permissions include contents/pull-requests write.
