...
last_edited: '2025-12-10T02:11:24.291Z'
next: ''
previous: ''
---
...
We have a created a public repo with a GitHub action you can use out of the box for auditing your content. Follow the [installation steps](https://github.com/tinacms/github-content-auditor?tab=readme-ov-file#how-to-install) to set it up.
...
Now locate the connection query for the collection you would like to audit. This query is auto generated when you add a collection to your schema and run/build your project.

`tina/__generated__/queries.gql`

> Note that query you provide must only return `_sys.path` and accept the `$first` and `$filter` properties. The filters will be supplied when the action is run and the path property will be used to locate your content.
...
Add your **Tina Content Query**, **Tina Token** and **Tina Client ID** to the `.env` file. These are the same properties added to your [GitHub Settings](/docs/guides/content-auditing-github-actions#secrets).
...
The workflow will also output a pull request to update the Last Checked date of every file that was audited.
