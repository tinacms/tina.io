...
last_edited: '2025-04-10T08:27:46.000Z'
next: ''
previous: ''
---
...
It is necessary to establish a trigger condition for this GitHub Action. Typically, the recommended approach is to trigger the translation process when a PR is successfully merged. If concerns exist regarding token consumption due to frequent triggering, an alternative option is to configure the GitHub Action to run on a scheduled basis. Below is an example of a GitHub Action that triggers only when a PR is successfully merged and contains file modifications within the specified directory.

```yaml
# Triggerd when the PR is successfully merged
on:
  pull_request:
    types: [closed]
    paths:
