...
As mentioned in [Introduction to Data fetching](/docs/features/data-fetching/), TinaCMS provides a client for querying content.
...
#### Filtering on Filename (\_sys)

Currently, the only way to filter documents by filename is by filtering an existing collection of documents via **code**.

For instance, to filter an array of posts to only include the file named `file-name-i-want`, you could use the following:
...
> Note: When using the auto-generated `<collection-name>Connection`, the query will only apply to the document's fields, not the `_sys` properties. Filtering in code as shown above is a suitable alternative.
...
TinaCMS supports cursor-based pagination:
