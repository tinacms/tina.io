...
Tina的实验性**通过ID选择表单**功能允许开发者在页面上存在多个`useTina`钩子（或查询）时选择在可视化编辑侧边栏中显示哪个表单。默认情况下，使用第一个请求的查询来确定表单。然而，通过使用此功能，您可以确保在Tina侧边栏中使用特定的表单——例如，与页面集合而不是全局配置相关联的表单。这在处理多个域时特别有用，因为您网站的不同部分或区域需要不同的表单配置。
