...
seo:
  ...
  description: >-
    学习如何在TinaCMS中使用React和Next.js设置可视化编辑。使用useTina钩子进行实时内容编辑、重新水合和高级表单选择以进行多重查询。
title: 可视化编辑设置
...
当您定义一个集合时，TinaCMS为您提供一个全页编辑器窗口来添加内容。

Tina在与React一起使用时还允许进行“可视化编辑”。这是一种设置，允许您的**网站和编辑器窗口并排显示**，实时显示您的更改。
...
为了在编辑时更新页面，您需要重新水合页面数据。在React（或基于React的框架如NextJS）中，您可以使用`useTina`钩子为可视化编辑配置页面。
...
> 如果您使用Tina客户端函数进行数据获取，这三个返回值可以直接插入到`useTina`钩子中。
...
Tina的实验性**通过ID选择表单**功能允许开发人员在页面上存在多个`useTina`钩子（或查询）时选择在可视化编辑侧边栏中显示哪个表单。默认情况下，使用第一个请求的查询来确定表单。然而，通过使用此功能，您可以确保在Tina侧边栏中使用特定表单——例如，与页面集合而不是全局配置相关联的表单。这在处理多个域时特别有用，因为您网站的不同部分或区域需要不同的表单配置。
