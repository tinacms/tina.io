...
1. `tina()` 集成的请求范围中间件缓冲每个 HTML 响应，并在编辑模式请求时，将桥接引导程序和每个 Tina 查询页面消耗的一个 `<div data-tina-form>` 负载拼接到 `<head>` 中。
2. 桥接（从 `/admin/bridge.js` 加载）读取这些负载，向父管理窗口发送 `open`，并在内存数据存储中播种。
3. 当编辑器输入时，管理端将 `updateData` 发送回 iframe。桥接存储它。
4. 页面中的每个可编辑区域都包裹在 `<TinaIsland>` 中，它发出一个 `<… data-tina-island="/tina-island/<name>?<params>">` 标记。在每次存储更新时，桥接将当前覆盖 POST 到该端点。
5. 端点根据覆盖数据重新渲染匹配的 Astro 组件并返回一个 HTML 片段。桥接将其替换到实时 DOM 中。
...
在 `astro.config.mjs` 中添加一次 `tina()`。该单次调用连接中间件（解析 `Astro.locals.tinaEdit` 并在编辑模式响应中注入桥接连接）并将原生 JS 桥接作为静态资产阶段化在 `/admin/bridge.js`。
...
`tinaAdminDevRedirect()` 是一个仅限开发的 Vite 插件，它将 `/admin` 和 `/admin/` 重定向到 `/admin/index.html`，以便在 `astro dev` 期间可以从裸 URL 访问管理 SPA。

没有共享的 `<head>` 连接组件，没有需要传递的 `forms` 属性，没有手动 `init()` 调用。集成处理所有这些。

## 数据加载器 — 用 `requestWithMetadata` 包装每个查询

每个路由的数据加载器调用生成的 Tina 客户端，并通过 `requestWithMetadata()` 管道传输结果。该单次调用：

- 将 `{ query, variables }` 哈希化为桥接用于寻址表单的稳定形式 ID。
- 从请求范围存储中读取桥接的覆盖，并在页面在管理 iframe 内渲染时将 `data` 替换为未保存的覆盖。
- 使用 `tinaField()` 需要的元数据标记结果，以便点击聚焦。
...
`priority: 'primary'` 将表单标记为页面的主要文档，因此编辑器在加载时打开它，而不是落在布局级别的全局（例如页眉/页脚配置）上。它镜像 `useTina()` 的 `experimental___selectFormByFormId`。在 SSR 页面上，第一个 `requestWithMetadata()` 调用会自动被视为主要；当您想要覆盖时，请显式传递它。

返回的对象具有 `{ data, query, variables, id }` — `data` 是您渲染的内容，而哈希化的 `id` 是桥接用于将覆盖匹配回表单的内容。
...
桥接在每次按键时 POST 到 `/tina-island/<name>`。一个动态路由，加上 `experimental_createIslandRoute()`，处理注册表中的每个条目：
...
这是桥接需要的唯一文件。助手强制同源 POST 使用 Tina-preview 内容类型，通过 Astro 的容器 API 渲染注册的组件，并将输出包装在注册的包装元素中——匹配页面侧的 `<TinaIsland>`，以便桥接可以将其替换。

## 在页面中使用可编辑区域

将每个可编辑区域包裹在 `<TinaIsland>` 中。`wrapper` 属性必须与注册表条目的包装器匹配（桥接替换整个元素）。标记页面的主要区域为 `primary`，以便管理在加载时打开该表单。
...
`tinaField()` 返回一个字符串，用于标识 DOM 元素对应的表单字段。在任何您希望在编辑器中可点击的元素上标记它：
...
<WarningCallout
  body={<>
    ...
    而不是从裸 `@tinacms/astro` 导入。Astro 的类型检查器通过子路径直接读取 `.astro` 文件；
    裸包默认通过 `types` 条件解析为 Astro 不识别为可渲染组件的占位符。
  </>}
/>
...
要在富文本正文中渲染自定义组件（例如，`YouTubeEmbed`），请编写两个文件：一个描述编辑器 UI 的模式 `Template`，以及一个与模板同名的 Astro 渲染器。
...
并在 `<TinaMarkdown components={…}>` 映射上注册渲染器：
...
> **两个 `name` 字符串必须匹配。** 模板的 `name: 'YouTubeEmbed'` 和组件映射键 `YouTubeEmbed` 是渲染器从富文本 AST 中调度 `mdxJsxFlowElement` 节点的方式。不匹配会将嵌入渲染为可见的占位符，以便您在开发期间发现缺失的注册。
...
支持 `output: 'static'`。上述中间件仅在按需渲染的路由上运行，因此在预渲染页面上它从不注入任何内容——相反，**`<TinaIsland>` 发出一个小的内嵌引导脚本**，仅在页面在管理 iframe 内打开时加载 `/admin/bridge.js`。启动时，桥接获取每个岛屿的 `/tina-island/[name]` 端点（仍然 `prerender = false`，因此适配器按需渲染它）以获取页面的表单负载，然后编辑工作与在 SSR 项目中完全相同。

静态编辑的要求：

- 将每个可编辑区域包裹在具有注册岛屿的 `<TinaIsland>` 中——这既是桥接重新渲染区域的方式，也是引导程序进入页面的方式。
- 在页面的主要 `<TinaIsland>` 上传递 `primary`。在静态页面上，桥接无法自动判断哪个岛屿是“页面”，因此没有这个，编辑器可能会落在多文档选择器上。
- 保持 `export const prerender = false` 在 `src/pages/tina-island/[name].ts` 上。

权衡：使用 `<TinaIsland>` 的页面在生产 HTML 中发送那一行内联引导——它不再与无 Tina 的 Astro 应用字节相同。没有 `<TinaIsland>` 的页面保持不变。
...
用逗号分隔以允许多个（预览 + 生产）。中间件内嵌它，以便桥接验证入站 postMessages。
...
| `@tinacms/astro/TinaMarkdown.astro` | `<TinaMarkdown content components />` — 富文本渲染器。从此子路径导入，以便 Astro 的检查看到一个真实的 `.astro` 组件。 |
| `@tinacms/astro/TinaIsland.astro` | `<TinaIsland name wrapper params [primary] />` — 可编辑区域的标记包装器 |
| `@tinacms/astro/integration` | `tina()` 集成 — 自动连接中间件并阶段化 `/admin/bridge.js` |
...
| `@tinacms/astro/vite` | `tinaAdminDevRedirect()` — 从 `/admin` 到 `/admin/index.html` 的仅开发重定向 |

## 另请参阅

* [Astro 启动模板](https://github.com/tinacms/tina-astro-starter)：参考实现，包含上述所有内容
* [部署到 Cloudflare Workers](/docs/tinacloud/deployment-options/cloudflare-workers)：发布站点并保持此按需岛屿路由在请求时工作
