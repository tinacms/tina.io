...
完成设置后，您可以访问 CMS，顶部会出现一个新的分支切换按钮。当您在受保护的分支上并点击“保存”时，会弹出一个模态框，提示您输入要保存的新分支名称。
...
这将允许您在将更改合并到受保护的分支之前预览您的内容更改。要设置预览链接，在您的配置文件中向 `ui` 对象添加一个 `previewUrl` 函数。此函数将接收分支名称作为参数，并应返回该分支的预览 URL。例如：
...
export default defineConfig({
  // ...
  ui: {
    // 例如，如果您正在部署到 Vercel，并且您的仓库名称是 'my-app'，Vercel 的预览 URL 将基于分支：
    previewUrl: (context) => {
