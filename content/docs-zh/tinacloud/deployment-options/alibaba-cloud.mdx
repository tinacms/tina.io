...
previous: content/docs-zh/tinacloud/deployment-options/cloudflare-workers.mdx
---

## 中国用户的替代部署方案

TinaCMS 主要通过 GitHub 和静态托管平台集成实现自动内容库更新，但**像 Vercel、Netlify 和 GitHub Pages 这样的静态托管平台在中国大陆无法访问**，且缺乏直接的中国替代方案。
...
鉴于这些限制，本指南将引导您将 TinaCMS 项目部署到 **阿里云**。作为中国领先的云服务提供商，阿里云为中国用户提供了可靠的性能。虽然目前没有中国云服务能够像 Vercel 那样无缝集成 GitHub，但阿里云仍然是希望在中国大陆部署网站的开发者的首选。

## 阿里云配置

阿里云提供了多种服务器选项。我们通常推荐使用 **弹性计算服务 (ECS)** 来部署 TinaCMS 项目。有关创建和配置服务器的详细说明，请参阅[阿里云官方文档](https://www.alibabacloud.com/help/en/elastic-compute-service)。

### 服务器设置

创建 ECS 实例后，您需要设置必要的软件来管理您的 TinaCMS 项目。

#### 安装 PM2 进行进程管理

PM2 是一个用于 Node.js 应用程序的进程管理器，有助于在生产环境中保持应用程序的运行。
...
Nginx 将作为您的 TinaCMS 应用程序的反向代理。

```
yum install nginx
```

接下来，启动 Nginx 并启用其在系统启动时运行：
...
首先，在您的服务器上创建一个目录，用于部署您的 TinaCMS 项目：
...
生成 SSH 密钥后，您需要将几个密钥添加到您的 GitHub 仓库中，以启用安全部署。
...
2. `SERVER_HOST`: 您的阿里云服务器的 IP 地址
   * 这是您 ECS 实例的公网 IP 地址
3. `SERVER_USERNAME`: 连接到您服务器的用户名
   * 通常是 `root` 或您在 ECS 实例上创建的用户帐户
4. `NEXT_PUBLIC_TINA_BRANCH`: TinaCMS 应该使用的内容分支
...
通过上述阿里云和 GitHub Actions 配置，中国用户现在可以在中国大陆正常使用所有 TinaCMS 功能。此设置保持与 Vercel 相同的自动内容同步，没有功能差异。