...
<WarningCallout
  body={<>
    `tinacms build` 需要在构建时提供环境变量，如 `TINA_TOKEN` 和 `NEXT_PUBLIC_TINA_CLIENT_ID`。确保这些变量在 Vercel 的环境设置中配置好。如果您使用像 Turborepo 这样的单体仓库工具，您可能还需要显式地转发这些变量（例如通过 `turbo.json` 中的 `env` 键）——否则即使它们在 Vercel 中设置了，`tinacms build` 也可能无法接收到它们。详情请参阅 [环境变量参考](/docs/reference/config#environment-variables)。
  </>}
/>
