...
<WarningCallout
  body={<>
    `tinacms build` 需要在构建时提供环境变量，如 `TINA_TOKEN` 和 `NEXT_PUBLIC_TINA_CLIENT_ID`。确保这些变量在 Vercel 的环境设置中配置好。如果您使用像 Turborepo 这样的单体仓库工具，您可能还需要显式地转发这些变量（例如通过 `turbo.json` 中的 `env` 键）——否则即使它们在 Vercel 中设置了，`tinacms build` 也可能无法接收到它们。详情请参阅 [环境变量参考](/zh/docs/reference/config#environment-variables)。
  </>}
/>

## 您的首次部署

准备好后，选择 `Deploy` 并等待您的首次构建运行。成功后，通过选择 `Inspect Deployment` 访问您的新 Tina 站点。

如果出现任何错误，请参考构建日志并相应地解决。

## 条件部署配置

默认情况下，每次您进行更改时，TinaCMS 都会推送一个带有更改的提交。这也意味着每次您进行更改时，Vercel 都会创建一个部署。这对于快速发布内容更改而无需手动部署的额外工作非常有用。如果该工作流程不适合您，请按照以下步骤配置 Vercel 的部署行为。

在 Vercel 中，转到您的项目的 **设置 | Git**。您会找到与 **`Deploy Hooks`** 和 **`Ignore Build Step`** 相关的选项。
...
在 Vercel 中，导航到 **我的账户 | 账户设置 | 令牌**，创建一个 **`范围`** 为您的项目的令牌，并设置适当的过期时间。给它一个描述性的名称。**在提示时复制令牌，您将在下一步中需要它，并且您将无法再次看到它。**

![令牌创建屏幕](/img/docs/tina-cloud-deployment-options/vercel-tokens.png "令牌创建屏幕")

然后导航到您的项目 **设置 | 常规 | 项目 ID** 并从那里复制 **`项目 ID`**。

![要复制的项目 ID 的位置](/img/docs/tina-cloud-deployment-options/vercel-project-id.png "要复制的项目 ID 的位置")

接下来，导航到您的项目并复制站点 URL，例如 [https://mycoolblog.vercel.app/](https://mycoolblog.vercel.app/)。

![要复制的项目 URL 的位置](/img/docs/tina-cloud-deployment-options/vercel-site-url.png "要复制的项目 URL 的位置")

最后，导航到 **我的账户 | 账户设置 | 常规 | 用户 ID** 并从那里复制 **`用户 ID`**。稍后将其用作组织 ID。

![要复制的用户 ID 的位置](/img/docs/tina-cloud-deployment-options/vercel-user-id.png "要复制的用户 ID 的位置")
...
将以下代码添加到创建的文件中。它将创建一个 GitHub Actions 工作流，当距离最后一次提交到 **`main`** 分支已经过去 5 分钟时，部署到 Vercel。请按照注释了解我们如何处理去抖动。

> 📝 **单体仓库注意事项**：您可能需要在工作流步骤中添加 `working-directory`
...
jobs:
  deploy:
    ...
    # - 'group': 定义并发的组。一次只能有一个作业在此组中运行。
    #            使用固定字符串如 'vercel-deploy-main' 意味着所有推送到 'main' 的作业
    #            都将属于此组。
    # - 'cancel-in-progress: true': 如果在此组中启动新作业时旧作业仍在运行，
    #                               旧作业将自动取消。
    concurrency:
