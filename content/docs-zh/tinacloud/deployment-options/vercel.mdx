...
必需的环境变量包括 NEXT\_PUBLIC\_TINA\_CLIENT\_ID 和 TINA\_TOKEN，您可以在您的 [TinaCloud 项目](https://app.tina.io) 中找到这两个变量。

<WarningCallout
  body={<>
    `tinacms build` 需要在构建时提供环境变量，如 `TINA_TOKEN` 和 `NEXT_PUBLIC_TINA_CLIENT_ID`。确保这些变量在 Vercel 的环境设置中配置好。如果您使用像 Turborepo 这样的 monorepo 工具，您可能还需要显式地转发这些变量（例如，通过 `turbo.json` 中的 `env` 键）——否则即使它们在 Vercel 中设置了，`tinacms build` 也可能无法接收到它们。详情请参阅 [环境变量参考](/docs/reference/config#environment-variables)。
  </>}
/>
...
您需要为您的主分支（例如，`main`）**禁用推送时的自动部署**。具体设置可能会根据您的 Vercel 计划或最近的 UI 更新略有不同，但目标是阻止 Vercel 直接监听您的 Git `push` 事件。以下指南概述了使其工作的 **`Ignore Build Step`** 选项。
    ...
    steps:
      ...
      - name: Debounce commits for 5 minutes
        run: |
          ...
          # 并且将启动最新提交的新作业。
          # 因此，如果此步骤完成，意味着在此分支上没有新提交的 5 分钟窗口。
