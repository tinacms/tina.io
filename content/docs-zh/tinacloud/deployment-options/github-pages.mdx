...
通过点击为我们创建的操作中的“Configure”，我们可以调整构建脚本以同时构建 tinacms 和我们的网站。
...
如果你的网站使用 GitHub Pages 部署，可能正在使用 GitHub 的默认构建步骤。在这种情况下，TinaCMS 不会包含在构建中。

![GitHub Pages 配置](/img/docs/tinacloud/gh-pages.png)。

要解决此问题，你需要选择 "GitHub Actions" 源，并同时构建 tinacms 管理界面和你的网站。你可以查看我们上面的文档以获取设置此 GitHub Action 的帮助。

#### 2. 'tinacms build' 未在 CI 中运行

检查以确保构建命令正在运行且没有失败

> 注意：如果你使用 [hugo 的 github pages 设置](https://gohugo.io/hosting-and-deployment/hosting-on-github/)，你需要确保在你的仓库根目录中存在一个 `package-lock.json`。
...
假设你的 TinaCMS `clientID` 和 `token` 已设置为环境变量，你需要将它们添加到项目的 GitHub Secrets 中。我们在上面的代码片段中使用的密钥是 `TINA_PUBLIC_CLIENT_ID` 和 `TINA_TOKEN`。