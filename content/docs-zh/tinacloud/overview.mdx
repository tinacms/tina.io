...
seo:
  ...
  description: >-
    学习如何使用 TinaCloud 将 TinaCMS 站点部署到生产环境。按照我们的分步指南，在 Vercel、Netlify 或 GitHub Pages 上进行设置和部署
...
要将您的站点部署到生产环境，您需要将 TinaCMS 连接到托管的后端。本文件将引导您完成从[本地运行到生产运行](/docs/introduction/faq/#what-is-local-mode-vs-prod-mode)的步骤。

## 前提条件

* 确保您的站点在 GitHub 上，并且所有更改都已推送（包括 `tina/tina-lock.json`）。此文件是通过在本地运行 `tinacms dev` 生成的——它必须被提交并推送，以便 TinaCloud 索引您的内容。
...
下一步是连接您的站点。一旦连接，您的项目编辑者将能够直接从站点内将内容保存到其 GitHub 存储库。
...
<WarningCallout
  body={<>
    注意：如果您从本地环境文件加载您的架构配置值，TinaCMS 的构建过程只会拾取 `.env` 文件（而不是 `.env.local` 或 `.env.development`）。
  </>}
/>
...
通常，您需要确保您的构建命令在站点的构建命令之前运行 `tinacms build`。这将与您的站点一起构建 TinaCMS 管理。您还需要确保您的 TinaCMS `NEXT_PUBLIC_TINA_CLIENT_ID` 和 `TINA_TOKEN` 被设置为主机上的环境变量。

<WarningCallout
  body={<>
    TinaCMS 管理是一个静态构建的 SPA —— 环境变量在构建时嵌入到其 JavaScript 中。您在管理中需要的任何变量（包括在[自定义字段组件](/docs/extending-tina/custom-field-components)中）必须：(1) 使用 `TINA_PUBLIC_` 或 `NEXT_PUBLIC_` 前缀，并且 (2) 在您的托管提供商的环境设置中配置，以便在 `tinacms build` 运行时可用。请参阅[环境变量参考](/docs/reference/config#environment-variables)以获取允许的前缀的完整列表。
  </>}
/>
