...
要将您的站点部署到生产环境，您需要将 Tina 连接到托管的后端。本文件将引导您完成从[本地运行到生产运行](/docs/introduction/faq/#what-is-local-mode-vs-prod-mode)的步骤。
...
下一步是连接您的站点。一旦连接，您项目的编辑者将能够直接从站点内将内容保存到其 GitHub 存储库。
...
* 用连接到 TinaCloud 所需的环境变量填充您的 `.env` 文件。
  * 如果您使用不同的环境变量文件，或者您已有 `.env` 文件，则需要手动复制这些变量。
...
<WarningCallout
  body={<>
    注意：如果您从本地环境文件加载您的架构配置值，Tina 的构建过程将只会拾取 `.env` 文件（而不是 `.env.local` 或 `.env.development`）。
  </>}
/>
...
通常，您需要确保您的构建命令在站点的构建命令之前运行 `tinacms build`。这将与您的站点一起构建 TinaCMS 管理。您还需要确保您的 Tina `NEXT_PUBLIC_TINA_CLIENT_ID` 和 `TINA_TOKEN` 被设置为主机上的环境变量。
