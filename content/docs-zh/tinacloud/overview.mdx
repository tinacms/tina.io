...
要将您的网站部署到生产环境，您需要将 Tina 连接到托管的后端。本文档将引导您完成从[本地运行到生产运行](/docs/introduction/faq/#what-is-local-mode-vs-prod-mode)的步骤。

## 前提条件

* 确保您的网站在 Github 上，并且所有更改都已推送（包括 `tina/tina-lock.json`）。
* 确保您有一个 [TinaCloud](https://app.tina.io) 账户。
  * 查看 [TinaCloud 仪表板](/docs/tinacloud/dashboard) 文档以获取更多信息。

## 创建一个 TinaCloud 项目

访问 [app.tina.io](https://app.tina.io) 并创建一个新的[项目](/docs/tinacloud/dashboard/projects/)。系统会要求您连接您的 Github 账户并选择要连接到 TinaCloud 的存储库。
...
您完全配置的 `tina/config.{js,ts}` 应该看起来像这样：
...
通常，您需要确保您的构建命令在网站的构建命令之前运行 `tinacms build`。这将与您的网站一起构建 TinaCMS 管理。您还需要确保您的 Tina `NEXT_PUBLIC_TINA_CLIENT_ID` 和 `TINA_TOKEN` 被设置为主机上的环境变量。
