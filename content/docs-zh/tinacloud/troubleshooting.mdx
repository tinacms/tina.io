...
如果您收到类似 `The specified branch, 'my-branch-name', has not been indexed by TinaCloud` 的错误，或看到没有分支被索引，请尝试以下操作：
...
在不使用[编辑工作流](/editorial-workflow)的情况下使用 TinaCloud 时，您需要确保 [TinaCloud App](https://github.com/apps/tina-cloud-app) 能够提交到仓库的选定分支。
...
通常这种情况发生在您的主分支上有分支保护，而 TinaCloud 无法绕过它。在 2024 年 7 月之前，TinaCloud GitHub 应用具有仓库的管理员权限，这使其能够绕过大多数仓库的这些规则。
...
Tina 当前使用的分支无效或未定义。仔细检查是否选择了正确的分支，并且它确实存在。
...
ContentAPI URL 格式不正确。请参阅我们的 [TinaCloud URL 格式指南](/docs/tinacloud/overview/#enabling-tina-cloud-in-tinacms) 以了解 URL 应如何格式化。
...
要解决此问题，请确保您的 TinaCloud 站点 URL 设置中指定的 URL 之一与 `?origin=` URL 参数中显示的 URL 完全匹配。
...
### 10. 如何在 VPN 或受限网络上解决 `Login attempt timed out` 错误？

TinaCloud 登录流程会通过多个外部域（AWS Cognito、API Gateway、GitHub）重定向，这些域可能会被您的 VPN 或防火墙阻止。如果重定向链中的任何域被阻止，登录将在完成前超时。
