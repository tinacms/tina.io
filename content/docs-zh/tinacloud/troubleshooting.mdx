...
如果在构建日志中出现此错误，意味着您部署的网站中的 `tina/tina-lock.json` 与 TinaCloud 中的版本不匹配。要解决此问题：
...
如果在访问 TinaCMS 界面时出现此错误，可能是由于项目中 `tinacms` 和 `@tinacms/cli` 的版本不匹配所致。更新这两个依赖项到最新版本，并在本地运行开发命令。提交对 `tina/tina-lock.json` 的任何更改，并将其推送到 TinaCloud 中链接的 git 仓库。
...
2. 验证在 `tina/config.ts` 中传递给 defineConfig 的配置属性中指定了正确的分支。注意，此值可能在您的 CI 构建过程中设置为环境变量
3. 验证分支是否存在于 GitHub 仓库中
4. 将分支拉入您的 TinaCloud 项目中。<Youtube embedSrc="https://youtube.com/embed/4fqTYtfpbBk" /> **视频：TinaCloud - 如何索引缺失的分支（3 分钟）**
...
通常这种情况发生在您的主分支上有分支保护，而 TinaCloud 无法绕过它。在 2024 年 7 月之前，TinaCloud GitHub 应用对仓库拥有管理员权限，这使其能够绕过大多数仓库的这些规则。
...
Tina 当前使用的分支无效或未定义。请仔细检查是否选择了正确的分支，并且它确实存在。
