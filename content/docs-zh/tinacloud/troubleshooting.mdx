...
如果您在构建日志中收到此错误，意味着您部署站点中的 `tina/tina-lock.json` 与 TinaCloud 中的版本不匹配。要解决此问题：
...
如果您在访问 TinaCMS 界面时收到此错误，可能是项目中 `tinacms` 和 `@tinacms/cli` 的版本不匹配所致。更新这两个依赖项到最新版本并在本地运行开发命令。提交对 `tina/tina-lock.json` 的任何更改，并将其推送到 TinaCloud 中链接的 git 仓库。

### 3. 如何解决由未索引分支引起的错误？

如果您收到类似 `The specified branch, 'my-branch-name', has not been indexed by TinaCloud` 的错误，或根本没有看到任何分支被索引，请尝试以下操作：
...
4. 将分支拉入您的 TinaCloud 项目。<Youtube embedSrc="https://youtube.com/embed/TatnNqKPKIQ?start=109" caption="" /> **视频：TinaCloud - 如何索引缺失的分支（3 分钟）**
5. 您还可以通过在该分支的 `tina/tina-lock.json` 文件中进行空白更改、提交并推送到 GitHub 来强制 TinaCloud 重新索引分支。这将启动分支的索引，几分钟后错误应得到解决
6. 确保 Tina 项目位于根目录中
...
通常这种情况发生在您的主分支上有分支保护，而 TinaCloud 无法绕过它。在 2024 年 7 月之前，TinaCloud GitHub 应用具有仓库的管理员权限，可以绕过大多数仓库的这些规则。
...
登录 TinaCloud 时，您尝试登录的网站的 URL 会通过 `?origin=` URL 参数发送。当认证窗口的 `?origin=` URL 参数中指定的 URL 与您的 TinaCloud 站点 URL 中定义的不匹配时，会出现此错误。
...
### 9. 如何解决在 VPN 或受限网络上 `Login attempt timed out` 错误？

TinaCloud 登录流程会重定向通过多个外部域（AWS Cognito、API Gateway、GitHub），这些域可能会被您的 VPN 或防火墙阻止。如果重定向链中的任何域被阻止，登录将在完成前超时。

要解决此问题，请确保您的网络允许出站流量到 [网络要求](/docs/tinacloud/network-requirements/) 页面上列出的所有域。仅将 `*.tina.io` 列入白名单是不够的——认证流程还需要访问 AWS 和 GitHub 域。