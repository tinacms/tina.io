...
如果在访问 TinaCMS 界面时出现此错误，可能是项目中的 `tinacms` 和 `@tinacms/cli` 版本不匹配导致的。更新这两个依赖项到最新版本并在本地运行开发命令。提交对 `tina/tina-lock.json` 的任何更改，并将其推送到 TinaCloud 中链接的 git 仓库。

### 3. 如何解决由未索引的分支引起的错误？

如果收到类似 `The specified branch, 'my-branch-name', has not been indexed by TinaCloud` 的错误，或者根本没有看到索引的分支，请尝试以下操作：
...
Tina 当前使用的分支无效或未定义。仔细检查是否选择了正确的分支，并且该分支确实存在。
...
要解决此问题，请确保在 TinaCloud 站点 URL 设置中指定的 URL 之一与 `?origin=` URL 参数中显示的完全匹配。
