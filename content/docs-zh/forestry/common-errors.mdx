...
> 在TinaCMS中，我们默认使用`_template`键而不是`template`键。Forestry迁移工具会自动处理这一点，为Forestry站点设置`templateKey`为"template"，但需要注意这一点。
...
在TinaCMS中，match属性的工作方式与在Forestry中有所不同。在Forestry中，match属性是一个包含文件扩展名的glob模式。在TinaCMS中，match属性是一个不包含文件扩展名的glob模式。
