...
以下是从Forestry迁移到TinaCMS时可能遇到的一些常见问题列表。

## 迁移包含非字母数字字符的字段

在TinaCMS中，所有字段只能包含字母数字字符或下划线。而在Forestry中，你可以有包含非字母数字字符的字段，例如`my-field-name`或`my field name`。
...
> Forestry迁移工具（`npx @tinacms/cli init`）在导入模板时应自动应用nameOverride。

## 迁移块

### \_template vs template

在TinaCMS中，使用块作为字段类型的概念（以前在Forestry中可用）被支持为[在对象字段中提供模板](/zh/docs/editing/blocks/)。在迁移过程中，迁移工具会将你的块转换为模板。

> 在TinaCMS中，我们默认使用`_template`键而不是`template`键。Forestry迁移工具会通过为Forestry站点设置`templateKey`为"template"来自动处理这一点，但需要注意这一点。

### 在块名称中使用破折号

在TinaCMS中，前置元数据模板的名称中通常会有`-`。TinaCMS本身不支持字段名称中使用破折号，但它支持使用`nameOverride`属性来处理这种情况。

例如，如果你有一个看起来像这样的前置元数据模板。
...
- template: call-to-action
#   ...
# 这假设你的块在块字段上设置了templateKey: 'template'。
- template: feature
#   ...
```
...
在Forestry中，你可以有一个块字段，其中包含一个未在前置元数据模板中定义的模板。在TinaCMS中，这是不允许的。你需要将Forestry中的模板添加到块字段的模板列表中，并为该模板定义字段。
...
这可能由于多种原因发生。一些常见原因是因为你尝试获取的数据

* 格式不正确
* 缺少必填字段
* 缺少`_template`键。
  如果你查看终端中的错误信息，你应该能看到导致问题的确切错误。

例如，如果你缺少`_template`，你将看到类似这样的错误
...
查看[match属性文档](/zh/docs/reference/collections/#matchinclude)以获取更多信息。
