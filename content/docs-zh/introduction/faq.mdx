...
它还允许不同类型的用户（内容创作者、编辑、市场人员等）进行授权登录和编辑他们的网站，而无需直接授予 GitHub 仓库的访问权限。
...
## 8. 为什么在使用 markdown 时需要数据库？

在使用 TinaCMS 的 markdown 时，数据库是必需的，因为服务于 Markdown 和 JSON 文件的数据层依赖于数据库来执行各种内容管理功能。虽然 Markdown 文件是您内容的主要来源，但数据库充当缓存，以启用高级功能，如搜索、分页和文件之间的交叉引用。它还提供了一个 API，允许像传统无头 CMS 操作一样获取内容。
...
支持！TinaCloud 可以通过在项目配置中指定 `tina` 文件夹的路径来处理 monorepos 中的站点。
...
### 用户已将开发 admin/index.html 推送到生产环境
...
如果您在访问 TinaCMS 界面时收到此错误，可能是由于项目中 `tinacms` 和 `@tinacms/cli` 的版本不匹配。更新这两个依赖项到最新版本并在本地运行开发命令。提交对 `tina/tina-lock.json` 的任何更改并将其推送到 TinaCloud 中链接的 git 仓库。

### 3. 如何解决由未索引分支引起的错误？

如果您收到类似 `指定的分支，'my-branch-name'，尚未被 TinaCloud 索引` 的错误，或者根本没有索引的分支，请尝试以下操作：
...
登录 TinaCloud 时，您尝试登录的网站的 URL 通过 `?origin=` URL 参数发送。当身份验证窗口的 `?origin=` URL 参数中指定的 URL 与您的 TinaCloud 站点 URL 中定义的 URL 不匹配时，可能会发生此错误。
