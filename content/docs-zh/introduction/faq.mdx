...
使用 TinaCMS 的 markdown 时，数据库是必需的，因为服务于 Markdown 和 JSON 文件的数据层依赖于数据库来执行各种内容管理功能。虽然 Markdown 文件是您内容的主要来源，但数据库充当缓存，以启用高级功能，如搜索、分页和文件之间的交叉引用。它还提供 API，允许内容获取类似于传统的无头 CMS 操作。
...
<WarningCallout
  body={<>
    **注意：** 本地模式是为了在本地开发而设计的，当您的网站托管在生产环境中时将不起作用。在本地模式下，Tina 尝试访问 `http://localhost:4001`，这在您的生产网站上运行时不可用（底层文件系统内容也不可用）。
  </>}
/>
...
无论您是 [自托管](/docs/self-hosted/overview) Tina 还是使用 [TinaCloud](/docs/tinacloud/dashboard/)，Tina 的内容 API 直接与 GitHub 进行身份验证，消除了用户创建 GitHub 帐户的需要。任何由您的编辑保存的更改都将提交到您 GitHub 仓库中配置的分支。
...
如果您在访问 TinaCMS 界面时收到此错误，可能是由于项目中 `tinacms` 和 `@tinacms/cli` 的版本不匹配。更新这两个依赖项到最新版本并在本地运行开发命令。提交对 `tina/tina-lock.json` 的任何更改，并将其推送到 TinaCloud 中链接的 git 仓库。
...
## 15. 如何在本地 GraphQL 服务器未运行或在生产环境中访问？
...
在选择要部署的入门模板并重定向到 Vercel 后，Vercel 添加其 GitHub 应用的弹出窗口可能无法打开。查看左下角的提示消息，显示“无法打开安装 GitHub 应用的弹出窗口”。
...
* 语义意义——格式化增加了清晰度和上下文
* 适合提示——一致的标记减少了歧义
* 人机可读——原始 Markdown 对人和机器都很友好
* 与 LLM 趋势一致——Markdown 自然适应向 LLM 优化内容的转变，正如 [SEO 已死，LLMO 长存](https://jina.ai/news/seo-is-dead-long-live-llmo) 中所强调的那样
