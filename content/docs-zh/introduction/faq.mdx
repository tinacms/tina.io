...
TinaCloud 允许在运行时更改内容分支，这一功能通过 URL 修改进行管理。然而，在自托管 Tina 中，分支更改仅限于构建阶段。
...
此错误表示 TinaCMS 管理 HTML 文件未能加载 JavaScript 包。通常由于以下几个原因导致：
...
通常发生这种情况是因为您的主分支上有分支保护，而 TinaCloud 无法绕过它。直到 2024 年 7 月，TinaCloud GitHub 应用对存储库拥有管理员权限，这使其能够绕过大多数存储库的这些规则。
