...
在比较 TinaCloud 和自托管 Tina 时，TinaCloud 有一些独特的功能。这些包括：
...
1. GitHub 作者化，以便将操作归因于 GitHub 账户。
2. [共同作者](https://docs.github.com/en/pull-requests/committing-changes-to-your-project/creating-and-editing-commits/creating-a-commit-with-multiple-authors) Git 提交，以便将提交归因于其他用户。

这两种方法都可以在 TinaCloud 账户设置中按用户启用。了解更多关于 [Git 作者化](/docs/tinacloud/git-co-authoring)的信息。

## 8. 为什么使用 Markdown 时需要数据库？

使用 TinaCMS 的 Markdown 时，数据库是必需的，因为提供 Markdown 和 JSON 文件的数据层依赖于数据库来执行各种内容管理功能。虽然 Markdown 文件是您内容的主要来源，但数据库充当缓存，以启用高级功能，如搜索、分页和文件之间的交叉引用。它还提供 API，允许内容获取类似于传统无头 CMS 的操作。
...
<WarningCallout
  body={<>
    **注意：** 本地模式用于本地开发，当您的网站托管在生产环境中时将不起作用。在本地模式下，Tina 尝试访问 `http://localhost:4001`，这在您的生产网站上运行时不可用（底层文件系统内容也不可用）。
  </>}
/>
...
无论您是 [自托管](/docs/self-hosted/overview) Tina 还是使用 [TinaCloud](/docs/tinacloud/dashboard/)，Tina 的内容 API 直接与 GitHub 进行身份验证，无需用户创建 GitHub 账户。您的编辑者保存的任何更改都将提交到您 GitHub 仓库中配置的分支。
...
如果您在访问 TinaCMS 界面时收到此错误，可能是由于项目中 `tinacms` 和 `@tinacms/cli` 的版本不匹配所致。更新这两个依赖项到最新版本，并在本地运行开发命令。提交对 `tina/tina-lock.json` 的任何更改，并将其推送到 TinaCloud 中链接的 git 仓库。

### 3. 如何解决由未索引分支引起的错误？

如果您收到类似 `指定的分支 'my-branch-name' 未被 TinaCloud 索引` 的错误，或者根本没有看到索引的分支，请尝试以下操作：

1. 验证默认分支上的 `tina/config.ts` 中是否存在配置文件
...
要解决此问题，请确保您的 TinaCloud 站点 URL 设置中指定的一个 URL 与 `?origin=` URL 参数中显示的完全匹配。
...
## 15. 如何在本地 GraphQL 服务器未运行或在生产中访问？
...
选择要部署的入门模板并重定向到 Vercel 后，Vercel 添加其 GitHub 应用的弹出窗口可能无法打开。请在左下角查找一条消息，提示“无法打开安装 GitHub 应用的弹出窗口”。
...
* 与 LLM 趋势一致 – Markdown 自然适应向 LLM 优化内容的转变，如 [SEO 已死，LLMO 长存](https://jina.ai/news/seo-is-dead-long-live-llmo) 中所述
