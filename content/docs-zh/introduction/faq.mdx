...
查看我们的社区：[tina.io/community](https://tina.io/community)。
...
使用 TinaCMS 的 Markdown 时，数据库是必不可少的，因为服务于 Markdown 和 JSON 文件的数据层依赖于数据库来执行各种内容管理功能。虽然 Markdown 文件是您内容的主要来源，但数据库充当缓存，以启用高级功能，如搜索、分页和文件之间的交叉引用。它还提供了一个 API，允许内容获取类似于传统的无头 CMS 操作。
...
无论您是 [自托管](/docs/self-hosted/overview) Tina 还是使用 [TinaCloud](/docs/tinacloud/dashboard/)，Tina 的内容 API 直接与 GitHub 进行身份验证，无需用户创建 GitHub 帐户。您的编辑者保存的任何更改都将提交到您 GitHub 存储库中配置的分支。
...
如果您在访问 TinaCMS 界面时收到此错误，可能是由于项目中 `tinacms` 和 `@tinacms/cli` 的版本不匹配所致。更新这两个依赖项到最新版本并在本地运行开发命令。提交对 `tina/tina-lock.json` 的任何更改并将其推送到 TinaCloud 中链接的 git 存储库。
...
5. 您可以通过在该分支的 `tina/tina-lock.json` 文件中进行空白更改来强制重新索引特定分支，提交更改并将其推送到 GitHub。这将启动分支的索引（几分钟后）错误应得到解决
6. 确保 Tina 项目位于根目录
