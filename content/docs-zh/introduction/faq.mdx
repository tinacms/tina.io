...
无论您是 [自托管](/docs/self-hosted/overview) Tina 还是使用 [TinaCloud](/docs/tinacloud/dashboard/)，Tina 的内容 API 直接与 GitHub 进行身份验证，无需用户创建 GitHub 账户。任何由您的编辑保存的更改都将提交到您 GitHub 存储库中配置的分支。
...
有关更多信息，请参阅 [Tina 路径](/docs/tinacloud/dashboard/projects/#path-to-tina)。
...
ContentAPI URL 格式不正确。请参阅 [此处](/docs/tinacloud/overview/#enabling-tina-cloud-in-tinacms) 了解 URL 应如何格式化。
