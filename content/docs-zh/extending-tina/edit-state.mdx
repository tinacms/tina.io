...
你可以通过使用 `useEditState` 钩子手动进入和退出编辑模式。你应该**始终**有一个 [`/pages/admin.{js,tsx}` 文件](/zh/docs/contextual-editing/overview)，但在某些情况下，你可能希望在不访问 `/admin` 或 `/admin#/logout` 路由的情况下进入编辑模式。
