...
自定义字段组件在 TinaCMS 管理界面中运行，该界面作为一个与您的网站分开的**静态 SPA**构建。环境变量在构建时嵌入到管理界面的 JavaScript 中——它们不会在运行时读取。这意味着要使变量可用，必须满足两个条件：
...
<WarningCallout
  body={<>
    如果您的自定义组件依赖于环境变量并且它总是 `undefined`，请检查：(1) 变量名称以 `TINA_PUBLIC_` 或 `NEXT_PUBLIC_` 开头，并且 (2) 变量在运行 `tinacms build` 时在您的 `.env` 文件或 CI 环境中定义。请注意，TinaCMS 仅加载 `.env` 文件——不加载 `.env.local` 或 `.env.development`。有关详细信息，请参阅[配置参考](/docs/reference/config#environment-variables)和[安全公告](/blog/2023-02-security-update)。
  </>}
/>
