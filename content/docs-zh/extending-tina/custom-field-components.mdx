...
自定义字段组件在 TinaCMS 管理界面中运行，该界面作为一个与您的网站分开的**静态 SPA**构建。环境变量在构建时嵌入到管理 JavaScript 中——它们不会在运行时读取。这意味着要使变量可用，必须满足两个条件：

1. **必须使用允许的前缀。** 出于安全原因，仅包括这些变量：
   - `TINA_PUBLIC_*`
   - `NEXT_PUBLIC_*`
   - `NODE_ENV`
   - `HEAD`
...
// 这将是 undefined——没有允许的前缀
const secret = process.env.MY_API_KEY
```

<WarningCallout
  body={<>
    如果您的自定义组件依赖于环境变量并且它总是 `undefined`，请检查：(1) 变量名称是否以 `TINA_PUBLIC_` 或 `NEXT_PUBLIC_` 开头，以及 (2) 变量是否在 `tinacms build` 运行时在您的 `.env` 文件或 CI 环境中定义。请注意，TinaCMS 仅加载 `.env` 文件——不加载 `.env.local` 或 `.env.development`。有关详细信息，请参阅 [配置参考](/docs/reference/config#environment-variables) 和 [安全公告](/blog/2023-02-security-update)。
  </>}
/>
...
> 可以从 [react-final-form FormAPI 文档](https://final-form.org/docs/final-form/types/FormApi)中推断出进一步的使用模式。
...
我们可以传递现有的 TinaCMS 图像组件，并通过 `useEffect` 拦截对它的更改，以更新我们的宽度和高度字段。
...
在内容模型中，我们还需要两个用于高度和宽度的附加字段，它们被设置为 `"hidden"`，因此不会出现在编辑器中。
...
通过一些额外的 CSS，您可以在 TinaCMS 中创建可缩放的图像。这将节省您为博客裁剪图像的时间。
![在 TinaCMS 编辑器中缩放图像](/img-scale-component.gif)
