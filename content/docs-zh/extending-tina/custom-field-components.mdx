...
通过在字段上设置 `ui.component` 属性，可以传递并渲染自定义组件。
...
如果提前知道图像的宽度和高度，NextJS 和其他框架能够提高性能。
...
这在编辑器中显示为单一的图像输入，但每次更新时将保存 3 个值（图像来源、高度和宽度）。
...
通过一些额外的 CSS，你可以在 Tina 中创建可缩放的图像。这将节省你为博客裁剪图像的时间。
