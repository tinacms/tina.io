...
TinaCMS运行后，导航到 `http://localhost:3000/admin/index.html`。
...
在使用应用路由的Next.js项目中使用TinaCMS时，您可能会遇到Vercel过于积极地缓存内容，导致某些更新未正确反映的情况。
...
有两种方法可以修改您的站点以最佳方式使用Vercel数据缓存：
...
在使用应用路由的Next.js项目中使用TinaCMS时，您可能会遇到新创建的内容项未立即出现在相应页面上的情况。这种行为通常是由Next.js和浏览器使用的积极缓存机制引起的。
