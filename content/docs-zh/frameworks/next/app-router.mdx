...
> 👆本指南假设您正在使用 **[Next.js 应用路由。](https://nextjs.org/docs/app)**
...
**有关配置的更详细概述，请参阅 [使用 TinaCMS 进行内容建模](/docs/extending-tina/overview/)**
...
在这里，我们使用 [generateStaticParams](https://nextjs.org/docs/app/api-reference/functions/generate-static-params) 将这些页面构建为 SSG。您可以根据需要自由更改此设置。
...
根本原因是 [Vercel 数据缓存](https://vercel.com/docs/data-cache) 正在缓存来自 TinaCloud 内容 API 的响应。这可以通过查询 GraphQL API 并查找更新的内容，或者检查 [Vercel 中缓存的使用情况](https://vercel.com/docs/data-cache/manage-data-cache)来确认。

有两种方法可以修改您的网站以最佳地使用 Vercel 数据缓存：

1. 手动更新路由段的默认重新验证时间。这将导致增量静态生成在运行时重新验证缓存的内容 API 响应时重建页面并更新内容。更多信息请参见：[https://nextjs.org/docs/app/api-reference/file-conventions/route-segment-config#revalidate](https://nextjs.org/docs/app/api-reference/file-conventions/route-segment-config#revalidate)
2. 向您的查询（TinaCMS 生成的）添加一个额外的参数，例如：\
   ...
   这将传递给底层的 `fetch` 请求。有关 Next.js 如何扩展 `fetch` 的更多信息，请参见：[https://nextjs.org/docs/app/api-reference/functions/fetch](https://nextjs.org/docs/app/api-reference/functions/fetch)
...
* [其他框架指南](/docs/frameworks/astro/) - Astro, Hugo, Gatsby 等