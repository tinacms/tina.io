...
在这里，我们使用 [generateStaticParams](https://nextjs.org/docs/app/api-reference/functions/generate-static-params) 将这些页面构建为SSG。您可以根据需要自由更改此设置。
...
在使用Next.js项目中的应用路由与TinaCMS一起工作时，您可能会遇到Vercel过于激进地缓存内容，导致某些更新未正确反映的情况。

根本原因是 [Vercel数据缓存](https://vercel.com/docs/data-cache) 正在缓存来自TinaCloud内容API的响应。这可以通过查询GraphQL API并查找更新的内容，或检查[Vercel中缓存的使用情况](https://vercel.com/docs/data-cache/manage-data-cache)来确认。
...
在使用Next.js项目中的应用路由与TinaCMS一起工作时，您可能会遇到新创建的内容项未立即出现在相应页面上的情况。这种行为通常是由Next.js和浏览器采用的激进缓存机制引起的。
