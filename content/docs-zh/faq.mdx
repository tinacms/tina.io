...
title: 常见问题
alias: 常见问题
last_edited: '2026-02-05T22:30:32.697Z'
next: ''
previous: ''
---
...
TinaCMS 支持所有 LTS 版本的 Node。请参阅 [Node.js Releases](https://nodejs.org/en/about/previous-releases) 了解更多关于发布版本的信息。

NPM、PNPM 和 Yarn 包管理器都被支持。

| Node 版本 |                                                                  |
| ------------ | ---------------------------------------------------------------- |
| v25.x        | ✅ 支持 - 当前版本但不是 LTS                                |
| v24.x LTS    | ✅ 支持 - 推荐用于生产                         |
| v23.x        | ℹ️ 支持 - 建议升级，因为 v23.x 已到达生命周期终点  |
| v22.x LTS    | ✅ 支持 - 适合生产                                |
| v21.x        | ℹ️ 支持 - 建议升级，因为 v21.x 已到达生命周期终点  |
| v20.x LTS    | ✅ 支持 - 推荐更新                              |
| v19.x        | ❌ 已到达生命周期终点                                                            |
...
这两种方法都可以在 TinaCloud 帐户设置中按用户启用。了解更多关于 [Git 授权](/docs/tinacloud/git-co-authoring)的信息。

## 8. 使用 markdown 时为什么需要数据库？

使用 TinaCMS 的 markdown 时，数据库是必不可少的，因为服务于 Markdown 和 JSON 文件的数据层依赖于数据库来执行各种内容管理功能。虽然 Markdown 文件是您内容的主要来源，但数据库充当缓存以启用高级功能，如搜索、分页和文件之间的交叉引用。它还提供了一个 API，允许内容获取类似于传统的无头 CMS 操作。
...
无论您是 [自托管](/docs/self-hosted/overview) Tina 还是使用 [TinaCloud](/docs/tinacloud/dashboard/)，Tina 的内容 API 直接与 GitHub 进行身份验证，无需用户创建 GitHub 帐户。任何由您的编辑保存的更改都将提交到您 GitHub 仓库中配置的分支。
...
当用户从您的网站登录时，我们会弹出一个登录窗口。当登录完成时，我们将尝试向主窗口发送消息。
...
在选择要部署的入门模板并被重定向到 Vercel 后，可能无法打开 Vercel 添加其 GitHub 应用的弹出窗口。请在左下角查找一条消息，提示“无法打开安装 GitHub 应用的弹出窗口”。
...
编辑者可以仅使用他们的电子邮件创建一个 [TinaCloud](https://app.tina.io) 帐户。如果为编辑者启用了共同作者功能，则提交将由他们的电子邮件地址共同署名。