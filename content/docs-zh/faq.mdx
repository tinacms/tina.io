...
使用 TinaCMS 的 markdown 时，数据库是必需的，因为服务于 Markdown 和 JSON 文件的数据层依赖于数据库来执行各种内容管理功能。虽然 Markdown 文件是您内容的主要来源，但数据库充当缓存以启用高级功能，如搜索、分页和文件之间的交叉引用。它还提供 API，允许内容获取类似于传统的无头 CMS 操作。
...
无论您是[自托管](/docs/self-hosted/overview) Tina 还是使用 [TinaCloud](/docs/tinacloud/dashboard/)，Tina 的内容 API 都直接与 GitHub 进行身份验证，无需用户创建 GitHub 账户。您的编辑者保存的任何更改都将提交到您 GitHub 仓库中配置的分支。
...
### 1. 弹出窗口拦截器阻止 Vercel 安装其 GitHub 应用
...
* 与 LLM 趋势一致——Markdown 自然适应向 LLM 优化内容的转变，正如 [SEO 已死，LLMO 长存](https://jina.ai/news/seo-is-dead-long-live-llmo)中所强调的
