...
在使用 TinaCMS 的 Markdown 时，数据库是必需的，因为提供 Markdown 和 JSON 文件的数据层依赖于数据库来执行各种内容管理功能。虽然 Markdown 文件是您内容的主要来源，但数据库充当缓存以启用高级功能，如搜索、分页和文件之间的交叉引用。它还提供了一个 API，允许内容获取类似于传统无头 CMS 的操作。
...
<WarningCallout
  body={<>
    **注意：** 本地模式是为了本地开发而设计的，当您的站点托管在生产环境中时将不起作用。在本地模式下，Tina 尝试访问 `http://localhost:4001`，这在您的生产站点上运行时不可用（底层文件系统内容也不可用）。
  </>}
/>
...
无论您是 [自托管](/docs/self-hosted/overview) Tina 还是使用 [TinaCloud](/docs/tinacloud/dashboard/)，Tina 的内容 API 都直接与 GitHub 进行身份验证，无需用户创建 GitHub 账户。任何由您的编辑者保存的更改都将提交到您 GitHub 仓库中配置的分支。
