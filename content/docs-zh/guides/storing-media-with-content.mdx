...
这种设置在许多现代 Web 框架（如 Next.js、Astro 等）中有效，其中静态资产从公共目录提供，但它破坏了**共址**——您的内容和相关资产位于不同的位置。

这在管理、复制或迁移内容时增加了摩擦。

## 解决方案：将您的内容移动到媒体文件夹中

当前解决此问题的方法是将 markdown 和静态资产都存储在 `public/uploads` 中。
