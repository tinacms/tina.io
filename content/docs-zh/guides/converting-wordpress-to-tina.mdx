...
* 您已设置[Tina入门](/docs/introduction/using-starter)
* 对使用[Tina编辑器](/docs/using-tina-editor)和[架构](/docs/schema)有基本了解
...
3\. 将您创建的集合导入`tina/config.tsx`中的[架构](/docs/reference/schema)。
...
使用[Tina编辑器](/docs/using-tina-editor)，您可以更新每个帖子的英雄图像。
...
3\. 在`app/posts/[...filename]/client-page.tsx`中，添加`coverImage`字段，以便可以使用[TinaMarkdown](/docs/editing/markdown#rendering-content-with-tinamarkdown)进行渲染。
