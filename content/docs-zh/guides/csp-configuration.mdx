...
* **s3.us-east-1.amazonaws.com** - 直接媒体上传。TinaCMS 媒体管理器在此主机上将文件 `PUT` 到预签名的 S3 URL。如果没有它，上传将在浏览器控制台中因 CSP `connect-src` 违规而失败。
...
对于页面路由，您也可以使用上述方法在 `next.config.js` 中配置 CSP。
...
如果您使用的是 [自托管 TinaCMS 设置](/zh/docs/self-hosted/overview/)，您可能需要调整这些 CSP 指令：
...
* [自托管设置](/zh/docs/self-hosted/overview/)
* [TinaCloud 概述](/zh/docs/tinacloud/overview/)
* [Next.js 框架指南](/zh/docs/frameworks/next/overview/)
