...
对于页面路由器，您也可以在 `next.config.js` 中使用上述方法配置 CSP。
...
2. **使用报告模式**：在强制执行之前使用 `Content-Security-Policy-Report-Only` 测试 CSP 更改
3. **定期审核**：随着应用程序的发展，审查并更新您的 CSP
