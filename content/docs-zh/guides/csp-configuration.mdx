...
TinaCMS 需要连接到多个服务以进行身份验证、内容交付、媒体上传和产品分析：
...
* **identity-v2.tinajs.io** - 身份验证和身份服务 (v2)。在 v1→v2 过渡期间，`identity.tinajs.io` 和 `identity-v2.tinajs.io` 都是必需的；管理 UI 根据端点调用两者。
* **content.tinajs.io** - 内容 API 和数据层
* **assets.tinajs.io** - 资产管理和交付
* **s3.us-east-1.amazonaws.com** - 直接媒体上传。TinaCMS 媒体管理器通过此主机上的预签名 S3 URL `PUT` 文件。没有它，上传将在浏览器控制台中因 CSP `connect-src` 违规而失败。
* **us.i.posthog.com** - PostHog 事件摄取。TinaCMS 管理 UI 发出产品分析事件以帮助我们改进编辑器。没有它，您将在浏览器控制台中看到编辑器尝试发送的每个事件的 CSP `connect-src` 违规。
* **us-assets.i.posthog.com** - PostHog 运行时资产（自动捕获、会话记录、功能标志）。由 `posthog-js` 懒加载；即使库本身已捆绑在管理 UI 中，也需要。
...
对于页面路由器，您也可以使用上述方法在 `next.config.js` 中配置 CSP。
...
如果您使用的是 [自托管 TinaCMS 设置](/docs/self-hosted/overview/)，您可能需要调整这些 CSP 指令：
...
* **阻止的内联样式**：对于某些 TinaCMS 功能，您可能需要 `style-src 'unsafe-inline'`
