...
上面的示例是一个Next.js API路由，但`TinaNodeBackend`本身并不依赖于Next。它是一个简单的Node `(req, res)`处理器，期望一个已解析的`req.body`，因此任何能够提供该对的东西都可以为其服务。
...
* 有问题吗？请随时在我们的[Discord](https://discord.com/invite/zumN63Ybpf)中询问。