...
last_edited: '2025-09-18T23:44:02.981Z'
...
对于不希望在 TinaCloud 上托管其 CMS 后端的用户，**我们提供了自托管选项**。这允许您运行自己的 TinaCMS 后端，灵活地提供自己的数据库、身份验证和 Git 集成，而无需依赖 TinaCloud。
...
TinaCMS 后端设计为兼容任何 Node.js 无服务器环境，例如 Vercel 或 Netlify。我们提供了一个可以一键部署到 [Vercel](https://vercel.com/) 的 [Next.js 入门](/docs/self-hosted/starters/nextjs-vercel)。您也可以部署到 [Netlify](https://www.netlify.com/) 或任何其他无服务器环境。
...
* [身份验证提供者](/docs/reference/self-hosted/auth-provider/overview)
  * 处理 CMS 操作的身份验证和授权。提供了一个默认的 [Auth.js](/docs/reference/self-hosted/auth-provider/authjs) 实现，支持您数据库中的用户集合。
...
> 注意：自托管 TinaCMS 时某些功能不可用。有关更多信息，请参见[自托管限制](/docs/introduction/faq#what-features-are-unavailable-in-self-hosted-tina-compared-to-tinacloud)部分。
