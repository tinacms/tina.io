...
本文档将指导您设置我们预配置的自托管示例库。此实现使用：

- [NextJS](https://nextjs.org/)
- 用于[数据库适配器](/zh/docs/reference/self-hosted/database-adapter/overview)的 [Vercel](/zh/docs/reference/self-hosted/database-adapter/vercel-kv) KV
- 用于其[身份验证提供者](/zh/docs/reference/self-hosted/auth-provider/overview)的 [NextAuth](/zh/docs/reference/self-hosted/auth-provider/authjs)
...
- 在您的 GitHub 账户中创建自托管入门[库](https://github.com/tinacms/tina-self-hosted-demo)的副本
...
可以在您的 [GitHub 开发者设置](https://github.com/settings/personal-access-tokens/new)中生成 GitHub 个人访问令牌。确保为新库分配 `repo` 访问权限，并具有对内容的读/写访问权限。

您可能希望将到期日期更改为超过 30 天（最大为 1 年）。

![GitHub 身份验证设置](/img/docs/self-hosted/personal-access-token.png)
...
查看 [用户管理文档](/zh/docs/self-hosted/user-management/)以获取有关如何添加其他用户的信息。
...
在本地使用与您之前在 Vercel 项目中设置的相同环境变量。
...
您将能够在 `http://localhost:3000` 查看您的入门站点。要使用 TinaCMS 编辑内容，请访问 `http://localhost:3000/admin`