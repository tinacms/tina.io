...
如[数据获取介绍](/docs/features/data-fetching/)中所述，TinaCMS 提供了一个用于查询内容的客户端。
...
> 注意：使用自动生成的 `<collection-name>Connection` 时，查询将仅应用于文档的字段，而不是 `_sys` 属性。如上所示的代码过滤是一种合适的替代方案。
...
在这里，我们将使用 postConnection 查询我们的帖子集合，并首先按类别排序，然后按日期排序，使用名为 category-date 的多字段索引：
