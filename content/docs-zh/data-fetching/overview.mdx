...
如[数据获取介绍](/zh/docs/features/data-fetching/)中所述，Tina 提供了一个用于查询内容的客户端。

> 注意，对于高级用例，您还可以手动查询底层的 [GraphQL API](/zh/docs/data-fetching/custom-queries/#writing-inline-queries)。
...
在客户端使用内置查询的一个注意事项是**您一次只能查询一个根集合**。如果您有一个页面上有多个表单，您可能需要使用[自定义查询](/zh/docs/data-fetching/custom-queries/)。
