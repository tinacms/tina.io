...
对于大多数简单情况，你应该能够利用TinaCMS客户端生成的查询来进行数据获取：
...
可以通过向`tina/queries`目录添加查询来扩展客户端以执行更高级的查询（包括一次查询多个根集合）。该目录中以`gql`或`graphql`结尾的所有文件都将被添加到客户端中。

TinaCMS生成自定义的[GraphQL片段](https://graphql.org/learn/queries/#fragments)以用于自定义查询，这样当模式更新时查询就不需要更新。
