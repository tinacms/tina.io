...
title: 引用字段
last_edited: '2026-06-17T06:47:35.254Z'
next: ''
previous: ''
---

<Youtube embedSrc="https://youtu.be/8D88hCDMUNk" />
...
> 有关所有字段类型的其他常见属性，请查看[字段](/zh/docs/reference/fields)类型定义。
...
在 Tina 中编辑时，用户将能够为 `author` 的值选择 `author` 集合中的一个文档。

![](/img/docs/reference/reference_field_example_tszsxd.png)

通过客户端或原始 GraphQL 查询 `post` 文档时，响应中的 `author` 键将包含*引用*的 `author` 文档的值：

<WarningCallout
  body={<>
    如果您引用的集合包含大量项目，则可能会遇到超时问题。查看 [FAQ - 加载引用字段时出现 503 服务不可用错误](/zh/docs/faq#18-getting-a-503-service-unavailable-error-when-loading-the-reference-field) 以获取更多信息。
  </>}
/>
...
如果您有一个很长的引用项目列表，可以使用引用集合中的属性通过 `ui.collectionFilter` 字段在模式中过滤它们。
...
默认情况下，引用选择器显示文档文件名。这可以通过 `ui.optionComponent` 设置为**自定义 React 组件**。

`optionComponent` 函数为您提供两个参数：`props` 和 `_internalSys`。

* `props` 代表引用集合中的字段。
* `_internalSys` 包含各种有用的信息供您在自定义组件中使用，例如 `file name` 和 `path`。
...
<WarningCallout
  body={<>
    如果您传入一个函数，它应该作为可执行函数传递。直接在回调中运行类似 `const url = new URL('https://bob-northwind-sydney.com')` 的操作是不允许的。
  </>}
/>
...
下面的示例显示了一个名为 `reference` 的集合，与具有引用字段的作者和帖子集合相关联。`ui.optionComponent` 属性定义了每个文档在 UI 中的标签显示方式。

`AuthorCollectionCustomReference` 和 `PostCollectionCustomReference` 是示例中未包含的自定义组件。
...
以 `AuthorProps` 接口为例，`optionComponent` 函数可以期望我们从 props 输入中使用的名称和描述。
...
// InternalSys 来自 tinacms，它提供了许多有用的信息供用户自定义他们的自定义组件
