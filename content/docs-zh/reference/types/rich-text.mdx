...
<WarningCallout
  body={<>
    `mark` 是一个保留的组件名称，用于内部的**高亮**功能。如果您创建一个名为mark的自定义MDX组件，它将与高亮功能冲突，并在编辑器中导致意外行为。
  </>}
/>
...
> 可视化编辑器目前不支持通过工具栏设置列对齐。如果需要在表格列中对齐内容，可以切换到原始Markdown模式并编辑表头分隔行中的对齐语法。
