...
> 注意：如果您尚未设置自托管，我们建议使用 `init backend` 命令。有关更多信息，请参阅[入门文档](/zh/docs/self-hosted/existing-site/)。
...
> 提示：在继续之前，请确保您已设置[数据库适配器](/zh/docs/reference/self-hosted/database-adapter/overview/)。
...
要添加其他用户，请导航到侧边栏中的用户集合以添加或编辑用户。有关更多详细信息，请参阅[用户管理](/zh/docs/self-hosted/user-management/)。
