...
[Clerk](https://clerk.com) 是一个用户管理服务，你可以在自托管的 TinaCMS 设置中使用它。
...
访问 [clerk.com](https://clerk.com/) 创建一个账户和一个“应用程序”。完成后，导航到 API Keys 标签以找到你的凭据，并将它们存储在项目中的 .env 文件中。

![Clerk API Keys 截图](/img/clerk-api-keys-screenshot.png)

确保用你将用于登录 Clerk 的电子邮件地址更新 `TINA_PUBLIC_ALLOWED_EMAIL`。
...
- 在 Clerk 中创建一个组织，并检查已登录用户是否是该项目组织的一部分。注意，免费计划的组织目前限制为 3 个用户。
