...
<apiReference
  ...
  property={[
    ...
    },
    {
      ...
      type: "AuthProvider",
      description:
        "设置[自定义身份验证](/zh/docs/reference/self-hosted/auth-provider/overview)。\n"
    },
    {
      groupName: "admin",
      name: "authHooks",
...
Tina 管理是一个**静态构建的 SPA**。环境变量在构建时（在 `tinacms build` 或 `tinacms dev` 期间）嵌入到管理的 JavaScript 中。它们不会在运行时读取。这意味着变量必须在构建运行时存在于环境中——之后设置它没有效果。

#### `.env` 文件支持

Tina 的构建过程仅从 `.env` 文件中获取变量。在 `.env.local`、`.env.development` 或其他 dotenv 变体中定义的变量**不会**被 TinaCMS 构建加载。如果你要部署到托管提供商（Vercel、Netlify 等），请确保变量也在该提供商的环境设置中配置。
...
在管理中通过 `process.env` 访问的任何其他环境变量——包括[自定义字段组件](/zh/docs/extending-tina/custom-field-components)、`beforeSubmit` 钩子或其他管理端代码——将是 `undefined`。
...
> 有关使用环境变量进行部署的更多信息，请参阅[使用 TinaCloud 进入生产](/zh/docs/tinacloud/overview)。有关管理变量作用域的安全背景，请参阅[2023 年 2 月安全公告](/zh/blog/2023-02-security-update)。
