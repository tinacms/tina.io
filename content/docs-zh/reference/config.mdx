...
<apiReference
  ...
  property={[
    ...
    },
    {
      ...
      description:
        "控制哪些网站可以与本地开发服务器通信。仅影响 `tinacms dev` — 在生产中无效。\n\n默认情况下，仅允许来自您自己机器（`localhost`）的请求。只有在远程环境（如 **GitHub Codespaces**、**Gitpod**、**Docker** 或 **WSL2**）中开发时才需要设置此项。\n\n每个条目可以是：\n\n- 一个 **URL 字符串** — 例如 `'https://my-codespace.github.dev'`\n- 一个 **正则表达式** — 用于匹配 URL 模式\n- `'private'` — 允许本地网络上的任何设备，即 RFC 1918 私有 IP 范围（对 Docker 或 WSL2 有用）\n\n```ts\nserver: {\n  allowedOrigins: ['private', 'https://my-codespace.github.dev']\n}\n```\n"
    },
    {
      groupName: "",
      name: "branch",
    ...
    },
    {
      ...
      type: "string",
      description:
        "用于本地开发的单独内容仓库的路径，**相对于 `tina/` 配置文件夹解析**（而不是项目根目录）。\n\n设置后，集合 `path` 值相对于此内容根目录解析。内容目录**必须包含一个 `tina/` 文件夹**（可以为空）— TinaCMS 在那里写入生成的文件，如 `tina-lock.json`。\n\n使用环境变量，因为路径可能因开发人员而异：\n\n```ts\nlocalContentPath: process.env.TINA_CONTENT_REPO_PATH // 例如 '../../my-content-repo'\n```\n\n有关完整的设置详细信息，请参阅 [单独内容仓库指南](/docs/guides/separate-content-repo)。\n"
    },
    {
      groupName: "",
      name: "authProvider",
    ...
    {
      description:
        '创建新文件夹时，文件夹名称必须匹配的正则表达式模式。仅适用于新创建的文件夹，不适用于现有文件夹。\n\n例如： "^\\[a-z0-9-]+$" 用于小写字母、数字和连字符\n',
      groupName: "ui",
    ...
    },
    {
      ...
      name: "fuzzyEnabled",
      type: "boolean",
      description: "启用时，即使查询中有小错误或拼写差异，搜索也会返回结果。\n",
      default: "true"
    },
    {
      groupName: "search.tina",
      ...
      type: "number",
      description: "一个单词与搜索词之间可以有多少字符更改（插入、删除、替换）仍然算作匹配。较高的值返回更多结果，但可能不太相关。\n",
      default: "2"
    },
    {
      groupName: "search.tina",
      ...
      type: "number",
      description:
        "结果与搜索词的相似度必须在 0 到 1 之间的阈值。提高此值以仅显示非常接近的匹配项；降低它以更宽松。\n",
      default: "0.6"
    },
    {
      groupName: "search.tina",
      ...
      type: "number",
      description:
        "搜索引擎将为每个搜索词考虑的相似词的最大数量。较高的值可以提高召回率，但可能会减慢搜索速度。\n",
      default: "10"
    },
    {
      groupName: "search.tina",
      ...
      type: "boolean",
      description:
        "启用时，交换字符（例如输入 \"teh\" 而不是 \"the\"）算作单个编辑而不是两个，使搜索对常见拼写错误更宽容。\n",
      default: "true"
    },
    {
      description:
...
这是一个使用 TinaCloud 托管数据层和内置 TinaCMS 媒体和搜索功能的标准配置。
...
TinaCMS 支持 Git 支持的媒体和特定的外部媒体提供商：
...
TinaCMS 管理是一个**静态构建的 SPA**。环境变量在构建时（在 `tinacms build` 或 `tinacms dev` 期间）嵌入到管理的 JavaScript 中。它们不会在运行时读取。这意味着变量必须在构建运行时存在于环境中——之后设置它没有效果。

#### `.env` 文件支持

TinaCMS 的构建过程仅从 `.env` 文件中获取变量。在 `.env.local`、`.env.development` 或其他 dotenv 变体中定义的变量**不会**被 TinaCMS 构建加载。如果你正在部署到托管提供商（Vercel、Netlify 等），请确保变量也在该提供商的环境设置中配置。
...
| 模式 | 描述 |
|---------|-------------|
| `TINA_PUBLIC_*` | TinaCMS 特定的公共变量 |
| `NEXT_PUBLIC_*` | Next.js 公共变量 |
| `NODE_ENV` | 当前环境（`development`、`production` 等） |
| `HEAD` | 当前分支（由 Netlify 使用） |

在管理中通过 `process.env` 访问的任何其他环境变量——包括 [自定义字段组件](/docs/extending-tina/custom-field-components)、`beforeSubmit` 钩子或其他管理端代码——将是 `undefined`。

<WarningCallout
  body={<>
    要使环境变量在管理中工作，它必须：（1）使用 `TINA_PUBLIC_` 或 `NEXT_PUBLIC_` 前缀，并且（2）在 `tinacms build` 或 `tinacms dev` 运行时设置在环境中。例如，将 `MY_API_KEY` 重命名为 `TINA_PUBLIC_MY_API_KEY` 并确保它在你的 `.env` 文件或 CI/托管环境变量中。
  </>}
/>
...
| 变量 | 上下文 | 描述 |
|----------|---------|-------------|
| `NEXT_PUBLIC_TINA_CLIENT_ID` | 站点 + 管理 | 你的 TinaCloud 项目客户端 ID |
| `TINA_TOKEN` | 仅站点构建 | TinaCloud 的只读令牌（不在管理中暴露） |
| `TINA_PUBLIC_IS_LOCAL` | 站点 + 管理 | 在自托管设置中在本地和生产后端之间切换 |
| `NEXT_PUBLIC_TINA_BRANCH` | 站点 + 管理 | 覆盖用于内容获取的分支 |
| `TINA_SEARCH_TOKEN` | 仅站点构建 | TinaCloud 搜索索引的令牌 |

> 有关使用环境变量进行部署的更多信息，请参阅 [使用 TinaCloud 进入生产](/docs/tinacloud/overview)。有关管理变量作用域的安全背景，请参阅 [2023 年 2 月安全公告](/blog/2023-02-security-update)。
