...
`static` 属性决定媒体文件是否可以通过编辑器直接上传、编辑或删除。
...
* 图像上传后不能更改，任何后续对资产的更改都不会反映。
* 如果您只有一个启用媒体的分支，媒体存储将从该分支获取/上传图像。
* 如果您有多个启用媒体的分支，则所有媒体将从/上传到仓库的默认分支。

如果您在非默认分支上配置 Tina（并且 Tina 配置尚未合并到您的默认分支），您可能会在 TinaCloud 媒体设置中看到此消息：

![](/img/docs/reference/media-not-configured_lnr5lw.png)

为了解决这个问题，您可能需要在将 Tina 合并到默认分支后启用媒体。