...
> 看不到你的媒体？对于新创建的项目，初始媒体同步会自动触发，可能仍在进行中。如果你的媒体仍未出现，你可以从 TinaCloud 项目仪表板的**媒体**选项卡手动触发同步。
...
将 `publicFolder` 设置为你网站的“公共”文件夹的路径。此路径是相对于你项目的根目录的。
...
> 此目录中的任何内容都将与 TinaCloud 的媒体服务器同步，并且图像将公开可访问。
...
如果你正在使用 Next.js 图像，你需要在 `next.config.js` 文件中添加类似以下内容，以允许访问外部图像。
...
基于仓库的媒体设计用于围绕单一分支工作流。如果你的项目正在使用[多个分支](/docs/tinacloud/branching/)或[编辑工作流](/docs/drafts/editorial-workflow/)，需要注意一些已知的注意事项。

* 图像不能被更改。一旦上传，任何后续对资产的更改都不会反映出来。
* 如果你只有一个启用媒体的分支，媒体存储将从该分支获取/上传图像。
* 如果你有多个启用媒体的分支，那么所有媒体将从/上传到仓库的默认分支。

如果你在非默认分支上配置 Tina（并且 Tina 配置尚未合并到你的默认分支），项目创建时的自动媒体同步可能无法找到你的媒体配置。在这种情况下，TinaCloud 项目仪表板中的**媒体**选项卡将显示媒体尚未同步。一旦你的 Tina 配置合并到默认分支，你可以从该选项卡手动触发同步。

## 重命名媒体文件

基于仓库的媒体不支持通过媒体管理器重命名文件——目前UI没有重命名操作。要重命名文件，你需要通过媒体库删除并重新上传，或直接在 GitHub 中进行更改。

> **预防措施：**避免使用带有空格、重音或特殊字符的文件名。使用小写 ASCII 和连字符（例如 `my-image.jpg`）是最安全的选择——带有特殊字符的文件名可能会破坏下游构建（例如 Astro）。上传时的文件名消毒功能正在 [tinacms#6359](https://github.com/tinacms/tinacms/issues/6359) 中跟踪。
...
**解决方案 2 — 直接在 GitHub 中重命名：**

1. 在 GitHub 上导航到你的仓库，并在你的媒体目录中找到媒体文件（例如 `public/uploads/`）
2. 直接在 GitHub 中删除旧文件
3. 等待删除同步回 TinaCloud
4. 直接在 GitHub 中添加新名称的文件
5. 等待新文件同步回 TinaCloud

> 如果跟踪的分支启用了分支保护规则，尝试通过媒体库上传将文件保存到媒体存储（S3），但更改将**不会**提交到受保护的分支。在这种设置中，请使用解决方案 2。

> 媒体库中的一流重命名支持正在 [tinacms#6722](https://github.com/tinacms/tinacms/issues/6722) 中跟踪。