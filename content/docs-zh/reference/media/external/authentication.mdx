...
Tina支持使用外部媒体提供商，但需要用户设置/托管一个轻量级的后端媒体处理器。Tina提供了一些助手来简化这一过程，适用于`cloudinary`、`s3`和`dos`（"Digital Ocean Spaces"）。
...
`authorized`键将确保只有TinaCloud中的授权用户可以上传和编辑媒体。
...
Vercel支持通过在项目根目录创建一个`/api`目录来创建无服务器函数。要设置此项，请遵循上述[NextJS特定说明](/zh/docs/reference/media/external/authentication/#option-1-nextjs-api-routes-nextjs-only)，但使用`/api/<YOUR_MEDIA_STORE_NAME>/[...media].ts`而不是`/pages/api/<YOUR_MEDIA_STORE_NAME>/[...media].ts`
...
首先，你必须设置重定向，以便所有对`/api/*`的请求都可以重定向到Netlify函数。你可以在`netlify.toml`中设置重定向。我们还将使用[esbuild](https://docs.netlify.com/configure-builds/file-based-configuration/#functions)来构建我们的函数，因此我们也将在`netlify.toml`中设置这一点。
...
1. 有了Lambda函数后，你可以继续创建一个API网关：
   1. 点击**`Create API`**
   2. 选择**REST API**
2. 一旦API创建完成，在根路径下创建一个名为**`/api`**的资源，方法是进入*Resources*并选择*Create Resource*
3. 接下来，创建一个嵌套的子路径，通过创建一个使用**`{proxy+}`**特殊语法的资源来获取所有**`/api`**子路径。确保勾选***Configure as proxy resource.***

   在设置ANY方法时，传递处理TinaCMS媒体管理器逻辑的Lambda函数。

   点击**`save`**并允许API网关向Lambda函数添加权限
4. 通过点击*Action*下拉菜单并选择Deploy API来部署你的API
   1. 为Deployment Stage选择**`[New Stage]`**并输入*Stage name*
   2. 一旦API部署完成，你可以在*Stages*菜单中点击你创建的阶段查看*Invoke URL*。
5. 通过进入*Settings*菜单并添加**`/*`**通配符来配置**Binary Media Types**

#### CloudFront

1. 为了完成连接，使用刚创建的API网关的*Invocation URL*为CloudFront创建一个新的**Origin**。将Origin Path设置为API部署的阶段名称。
2. 为CloudFront创建一个新的**Behaviour**，该行为将拦截带有**`/api/s3/media*`**路径的请求，并使用刚创建的API网关origin。确保允许以下HTTP方法：**`GET`**、**`HEAD`**、**`OPTIONS`**、**`PUT`**、**`POST`**、**`PATCH`**和**`DELETE`**。
...
3. 重复上述过程以创建另一个行为，该行为将拦截带有**`/api/s3/media/*`**路径的请求。
...
> 确保在推送到TinaCloud的生产环境时同时提交对配置和[`tina-lock.json`](/zh/docs/tina-folder/overview/#tina-lockjson)文件的更改，否则你的资产仍将以`https://assets.tina.io`为前缀，就像你[仍在使用基于仓库的媒体](/zh/docs/reference/media/repo-based/)一样。
