...
```sh
  ...
  dev [options]          构建 TinaCMS 并启动开发服务器
...
| `--noWatch`               | 不在文件更改时重新生成配置。此选项适合在 CI 中构建时添加，或者如果您不想监视文件系统的更改。                                                                          |
| `--noSDK`                 | 不生成生成的客户端 SDK。[阅读此处](/docs/graphql/cli/) 了解有关 SDK 的更多详细信息。                                                                                  |
...
`dev` 将把架构编译成静态文件，为您的项目生成 TypeScript 类型，并在 [http://localhost:4001](http://localhost:4001) 上启动一个 graphQL 服务器。
...
此命令编译并验证架构，生成客户端和类型。它还将确保您的内容已完成索引。
...
* 在构建期间将您的内容索引到临时内存数据层中
* 生成**生产** TinaCMS 客户端（因此部署的网站在运行时仍与 TinaCloud 通信）
* 在构建期间进行**零** TinaCloud 内容 API 请求
* 运行您的框架的构建命令

当 SSG 生成数千个页面时很有用。在 SSW Rules（约 4,000 页）上，构建时间从 4:55 降至 2:12，TinaCloud 内容 API 调用从约 4,000 次降至 0 次。

由于生成的客户端是生产客户端，`branch`、`clientId` 和 `token` 必须在您的 TinaCMS 配置中配置，与正常 `tinacms build` 使用的值相同。

需要了解的一些事项：

* 在构建期间跳过搜索索引（会发出警告）。如果依赖搜索，请单独重新索引。
...
`doctor` 命令检查您项目中安装的 TinaCMS 包是否与 npm `latest` dist-tag 一致。它回答了“我是否是最新的？”这个问题——CLI 对应于 [`/versions`](/versions) 页面。

该命令：

* 读取您的 `package.json` 并从 `node_modules` 或您的项目锁定文件中解析已安装的版本（支持 `package-lock.json`、`pnpm-lock.yaml` 和 `yarn.lock`）。
* 过滤到 TinaCMS 包系列：`tinacms`、`@tinacms/*`、`tinacms-*`、`next-tinacms-*` 和 `create-tina-app`。
