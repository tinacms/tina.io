...
由于生成的客户端是生产客户端，因此必须在您的 TinaCMS 配置中配置 `branch`、`clientId` 和 `token`，这些值与正常的 `tinacms build` 使用的值相同。
...
`doctor` 命令检查项目中安装的 TinaCMS 包是否与 npm `latest` dist-tag 相符。它回答了“我是否是最新的？”这个问题——这是 CLI 对应于[`/versions`](/versions)页面的功能。
