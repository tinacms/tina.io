...
draft: false
seo:
  title: Deprecating TinaCMS-Git-Server | TinaCMS Blog
  description: >-
    Learn about the deprecation of TinaCMS-Git-Server in TinaCMS and what it means
...
We've changed our recommended approach for using TinaCMS's Git Backend with Next.js websites. Check out [TinaCMS's Next.js documentation](/docs/reference/self-hosted/tina-backend/nextjs) for details.

To make it easy to use TinaCMS's backend plugins, they are designed as Express middleware. These can be easily attached to Gatsby's dev server by implementing Gatsby's `onCreateDevServer` method and adding a few lines of connecting code. We reasoned that we could attach the same middleware to the dev server in other frameworks, allowing us to run the same backend code without having to rewrite it for each framework we intend to support.

With Next.js, the lack of a plugin system requires developers to write a bit more code in order work with TinaCMS. In order to use Express middleware with a Next.js site, a developer can do one of two things:
...
Ultimately, solution 2 is both simpler and more robust. Although it requires more work on the end of the developer using TinaCMS, this is often a side effect of software that properly [inverts control](https://kentcdodds.com/blog/inversion-of-control/).
