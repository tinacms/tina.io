...
There's more context in the [latest sprint review](https://youtu.be/bhjE5i0y8VY) (@17:00) if you want it.
...
Astro lets a single route opt into server rendering while the rest of the project stays static. In the starter, `/tina-island/[name]` declares `prerender = false` and runs server-side. Everything else stays `output: 'static'` — prerendered at build time and served from a CDN.

When the editor sends unsaved data to that route, the server re-renders the affected component with the overlay data and returns the HTML fragment for the bridge to swap in. Only the editing path pays the SSR cost. Before this, the only way to get a live preview was to ship React and hydrate the page in the editor — which is what the old Astro setup did.

## What visitors actually load

On pages with editable regions there's a small inline `<script>` that checks `window.self !== window.top` and exits immediately outside the editor iframe — a few dozen bytes, no network request.
...
Each editable region is wrapped in a `<TinaIsland>` that emits a `data-tina-island="/tina-island/<name>?…"` attribute. The bridge POSTs the unsaved form data to that URL, the endpoint calls `readOverlay()` and renders the component server-side with the overlay applied, and the bridge swaps the response HTML in. The endpoint never reads from the content store on that path. Updates debounce into a single request per 300ms.
...
Add an entry to `islands.ts` and reference it from a page with `<TinaIsland name="...">`. The `experimental_` prefix is real — these APIs may change before they stabilise.
...
Diff against the starter when you're ready: [tina.io/docs/frameworks/astro/](https://tina.io/docs/frameworks/astro/).

## Other frameworks

A Nuxt or Eleventy adapter would follow the same bridge protocol pattern, but we haven't built one. We're happy to review pull requests for new framework adapters.
