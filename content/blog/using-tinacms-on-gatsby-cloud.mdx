...
seo:
  title: Using TinaCMS on Gatsby Cloud | TinaCMS Blog
  description: >-
    Learn how to integrate TinaCMS with Gatsby Cloud for a seamless, fast, and
    scalable content management experience with Gatsby-powered websites.
title: Using TinaCMS on Gatsby Cloud
date: '2020-01-03T12:00:00.000Z'
author: James O'Halloran
prev: content/blog/editing-on-the-cloud.mdx
next: content/blog/three-ways-to-edit-md.mdx
---
...
Since Gatsby Cloud is built from the ground up specifically for Gatsby sites, it takes advantage of *build-artifact caching* & *parallelizing tasks* to make things fast ⚡

So, let's do this! 🕺

## Deploying a Preview on Gatsby Cloud 🚀

Deploying a preview with Gatsby Cloud can be done in just a few clicks. Once we've created an account, we can connect the site's repository and enter some build information (the site's root directory and environment variables).
...
Now that our **preview is live**, there's some extra configuration that we'll want to do to have TinaCMS work smoothly on the cloud.
...
To get TinaCMS working in Gatsby Cloud, we'll need to configure a few environment variables in our **Gatsby Cloud site settings**:
...
This needs to be set to ensure that TinaCMS knows that it is being run in a *Cloud Editing Environment*. Set it to `true`.
...
When it's time to get our editors editing a TinaCMS site on a cloud editing environment, we will need to be **extra careful around safeguarding against run-time errors.** Does the site blow up if an array is empty? or if an image isn't defined? We'll want to account for these edge cases which your editors may run into while editing.

## Happy Cloud Editing! ☁️

Hopefully this gets you started editing **your Gatsby site with TinaCMS on the cloud.**
...
If you run into trouble or have any questions, head over to the [TinaCMS Forum](https://community.tinacms.org/) for help. Stoked on TinaCMS? Please ⭐️ us on [GitHub](https://github.com/tinacms/tinacms) or [Tweet us](https://twitter.com/Tina_cms) 🐦 to show-off your TinaCMS projects.
