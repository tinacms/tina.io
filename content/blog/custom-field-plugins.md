...
At a minimum, field plugins **require a name and a component.** The `name` will be used to reference the custom field in form definitions ([more on this later](https://tinacms.org/blog/custom-field-plugins#2-use-the-custom-field-in-a-form)). The `Component` is what will actually be rendered in the sidebar.
