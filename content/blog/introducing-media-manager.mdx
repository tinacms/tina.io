---
seo:
  title: Introducing Media Manager | TinaCMS Blog
  description: >-
    Discover the new Media Manager in TinaCMS, designed to simplify media
    organization, improve file management, and enhance your content workflow.
title: Introducing the TinaCMS Media Manager
date: '2020-10-02T10:20:39-04:00'
last_edited: '2020-10-05T01:23:41.347Z'
author: Kendall Strautman
prev: content/blog/september-update.mdx
next: content/blog/more-changes-coming-to-inline-editing.mdx
---
...
Up until now, media changes with TinaCMS have been handled through image fields. By clicking on the field, editors could upload new images from their local filesystem. Developers would register a [Media Store](/docs/reference/media/overview) to the CMS that would handle uploading files. However, there was no way to view images that had already been uploaded to use them in a piece of content.
...
3 new attributes were added to the individual [`Media` object interface](/docs/reference/media/overview).
...
Two new methods were added to the [Media Store](/docs/reference/media/overview) interface.
...
Another improvement is the ability to [extend styles](/docs/contextual-editing/overview) on the inline image. The inline image field adds a few elements to the DOM, which could throw off styles for some layouts. Styles can now be set directly via `className` or through [styled-components](https://styled-components.com/docs/basics#extending-styles).
...
We unified the `previewSrc` interface between [`MediaStore`](https://tina.io/docs/reference/media/overview/), [`InlineImage`](/docs/contextual-editing/overview), and the regular [image field](/docs/plugins/fields/images).
...
Media stores can implement a catch-all `previewSrc` method that will be run if a field does not define its own `previewSrc`. If you're using a media store provided by a TinaCMS package, or using one that otherwise defines a `previewSrc`, you can probably remove your field's custom `previewSrc` method entirely.
...
Instead of passing the `filename`, the entire [media object](/docs/reference/media/overview) is now passed. As a general recommendation, you could use either `media.id`(the full path to the image) or `media.filename` to adjust the preferred final value saved in the data source.
...
### *InlineImage* render child props
...
> Note that if you're using [gatsby-image](https://www.gatsbyjs.com/plugins/gatsby-image/), you'll still need to provide a backup `src` value as the path to the image is *transformed* and does not reflect the value in the data source.
...
Hopefully, this post arms you with all the information needed to upgrade your TinaCMS packages and start playing with the media manager. As usual, we'd love feedback and ideas in the [Discord](https://discord.com/invite/zumN63Ybpf) or [bug reports](https://github.com/tinacms/tinacms/issues) on the repository. Thanks and happy media managing 🦙 🌅!
