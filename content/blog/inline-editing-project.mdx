---
warningMessage: >-
  **Update:** Inline editing is evolving and the current version is considered
  experimental, read more about [the evolution of inline
  editing](/blog/evolution-of-inline-editing/).
seo:
  title: Inline Editing Revamped | TinaCMS Blog
  description: >-
    Learn about Inline Editing in TinaCMS, which allows for real-time content
    updates directly on your site, streamlining the editing process.
title: Inline Editing Revamped
date: '2020-06-29T11:28:19-03:00'
author: Kendall Strautman
prev: content/blog/upgrade-notice-improved-github-security.mdx
next: content/blog/september-update.mdx
---

6 weeks ago, we embarked on a dedicated project with the goal of improving [Inline Editing](/docs/contextual-editing/overview). We believe that **this way of creating content is the future**, and we want to be a driver in actualizing that future.

<Youtube embedSrc="https://www.youtube.com/embed/4qGz0cP_DSA" />

We chose to specifically focus on [Inline Blocks](/docs/editing/blocks), to hone in on the page builder capacities of TinaCMS. What follows are some **improvements and changes** made within react-tinacms-inline, along with process notes gathered throughout the course of this project:
...
We surfaced the ability to precisely control the amount of this offset by passing 'x' and 'y' values to the focus ring configuration. And we provided an `insetControls` prop that will tuck the group or block controls *within* the upper corner of the element.
...
This feature allows designers & developers to craft flexible layouts for editors while employing *guardrails to uphold their design*.
...
Something that was important to us was supporting [nested blocks](/docs/editing/blocks) well — meaning blocks can render other blocks without the controls and focus states conflicting.
...
### New [Inline Group Field](/docs/editing/blocks)

Before this field was introduced, the only way to edit metadata in the *Settings Modal* was through a block. This field provides an opportunity to add blocks-like controls and a focus ring to a group of regular inline fields.
...
We introduced a *confirm* button and adjusted the functionality of the cancel button so it actually resets form changes. This clarity for editors to better control and implement changes to the settings form.

![settings-modal](/img/inline-blocks/settings-modal-blog.jpg)

### Enabling edit mode through the CMS

During the course of this project, the ability to [enable or disable](/docs/contextual-editing/overview) the CMS entirely was introduced to the project. With this feature, we can now control *Inline Editing Mode* through this `enabled` flag. When the CMS is enabled, the inline editing state is automatically active.

This simplifies inline editing configuration. Developers no longer need to make a separate 'Enable Edit Mode' toggle to *engage* that active editing state. They can control whether the CMS is enabled throughout their entire project at the App level.
...
We want to give developers the freedom to build fully custom page-builder type workflows. While we have some ideas already on how to take this further, **we'd love to hear your feedback**! Feel free to reach out with ideas on [Twitter](https://twitter.com/tinacms).

To dive deeper, please read [our blocks documentation](/docs/editing/blocks) to get more familiar with the technical implementation of these new features. Or play around with the [simple demo](https://logan-anderson.github.io/cra-hosted-demo/) made from [the guide](/docs/editing/blocks) to get inspired!
