...
seo:
  title: Custom Field Components | TinaCMS Blog
  description: >-
    Learn how to build custom field components in TinaCMS to enhance content
    editing with personalized, flexible fields that suit your project.
title: How to Make a Custom Field Component
date: '2020-01-20T07:00:00.000Z'
author: Kendall Strautman
prev: content/blog/exporting-wordpress-content-to-gatsby.mdx
...
> Want to jump ahead? Feel free to check out a **[finished version](https://github.com/kendallstrautman/llama-filters)** of the custom range input field seen in the gif above, or take a peak at a more complex [*Authors* field plugin](https://github.com/tinacms/tina-starter-grande/blob/master/src/fields/authors.js) in the Tina Grande repo.
...
Now this example component is super simple — a glorified label. This type of component can be helpful with organizing or customizing the sidebar, but *we can go further and pass in more complex fields*.

## Custom Range Slider 🎨

Say we had an image on the *About Me* page and we wanted to be able to control some [CSS filters](https://css-tricks.com/almanac/properties/f/filter/) on that image. The pen below shows all the CSS filters we have to play with.
...
> **Tip:** Saturation in photography relates to the *intensity of particular colors in an image*. A highly saturated image would be very bright, with colors bordering on neon. An image with low saturation would appear muted and grey.
...
* `input` — The object with data and callbacks for the field to set and update data. *Outlined above* ☝️.
...
In this step, we need to create the custom field definition and pass in the `RangeInput` component inline. We'll go back to our *About Me* page [form options](/guides/gatsby/git/customize-form):
...
Some other examples of awesome *CSS-in-JS* frameworks are [styled-components](https://www.styled-components.com/) and [emotion.js](https://emotion.sh/docs/introduction). Note that the above implementation for these alternative frameworks this will look slightly different.
...
> Feel free to check out a [finished version](https://github.com/kendallstrautman/llama-filters) of this custom range input field, or take a peak at a more complex [*Authors* field plugin](https://github.com/tinacms/tina-starter-grande/blob/master/src/fields/authors.js) in the Tina Grande repo.
