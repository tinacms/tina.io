...
You can use the data from `create-form` to creatively generate new file paths or populate default data (more on that later). Overall, formatting the `filename` depends on the structure of your project. Pick a simple solution that makes sense to you or checkout [more examples in the GitHub provider docs](/docs/reference/self-hosted/git-provider/github).
