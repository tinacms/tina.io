---
seo:
  title: Evolution of Inline Editing | TinaCMS Blog
  description: >-
    Explore the evolution of inline editing in TinaCMS, from its early stages to
    the current version that streamlines content updates directly on the page
...
prev: content/blog/using-graphql-with-the-filesystem.mdx
next: content/blog/tina-cloud-and-nextjs-the-perfect-match.mdx
---
...
And let's not forget about sidebar editing in this discussion! The attractiveness of inline editing often overshadows the flexible setup and usability wins that we get with sidebar editing. **We recommend starting with [sidebar editing](/docs/using-tina-editor) to get up and running quickly.**

## A review of the original approach

Let's take a look at a simple [inline editing](/docs/contextual-editing/overview) scenario where we make the title of a page editable.
...
`useFieldRef` is the first part of an experimental new API for creating a ref-based inline editing experience with TinaCMS. With `useFieldRef`, inline editing components are defined in the form configuration, and you assign a [ref](https://react.dev/learn/referencing-values-with-refs#refs-and-the-dom) to the component in your layout that the field should attach to.
...
You can create editing-specific routes if you are concerned about the performance implications of TinaCMS being bundled with your website. Frameworks like [Next.js](https://nextjs.org) optimize Javascript bundles so that TinaCMS is *not* included on routes that don't need it.
...
Moreover [TinaCloud](https://app.tina.io) auto-generates sidebar forms based on your content's schema and represents our current recommended best practice.
