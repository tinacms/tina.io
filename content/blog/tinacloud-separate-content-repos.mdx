...
next: content/blog/tinacms-news-may-june-2026.mdx
---

TinaCloud projects can now bind **two** GitHub repositories: a **generator repo** that holds your TinaCMS schema and application code, and a separate **content repo** that holds your markdown, MDX, and media.
...
Until now, the workaround was to create two TinaCloud projects and wire them together with a `tina/meta.json` file. That pattern worked, but it came with sync issues, duplicate TinaCMS lock files, and search results that could drift out of date. Separate content repos replace it with a single project that understands both repos natively.
...
* **Editorial branches and pull requests** created through TinaCMS's editorial workflow are made on the content repo.
...
![The new-project flow with the separate content repository toggle enabled](/img/blog/tinacloud-separate-content-repos/seperate-content-repo-new-project.png)
...
![The project configuration page showing the content repository section](/img/blog/tinacloud-separate-content-repos/seperate-content-repo-existing-project.png)
