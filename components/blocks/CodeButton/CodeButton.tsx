...
export const CodeButton = ({
  ...
  console.log('Pop Over Text', popOverText?.children?.length > 0);
  ...
  useEffect(() => {
    ...
    if (hasSession) setShowPopover(true);
  }, [sessionKey]);
    ...
    <div className="relative inline-block">
      ...
      {showPopover && popOverText?.children?.length > 0 && (
        <div className='absolute flex flex-row items-center gap-4 top-full left-0 mt-2 z-10 rounded-md bg-white p-4 shadow-md border border-seafoam-150'>
          ...
          <TinaMarkdown content={popOverText} components={CodeButtonMarkdownStyle} />
          <button onClick={() => setShowPopover(false)} className='hover:text-orange-500 hover:cursor-pointer'>
