            ...
            ui: {
              component: wrapFieldsWithMeta(({ input }) => {
                ...
                useEffect(() => {
                  ...
                  const parsedValue = value.map((item) => {
                    try {
                      ...
                      return { columnHeader: 'Unknown', cellValue: '', isTicked: false };
                    }
                  });
                ...
                const updateCellValue = (index, newValue) => {
                  ...
                  input.onChange(stringifiedValue); // Propagate changes
                };
                    ...
                    {valueMap.map((cell, index) => (
                      <div className="mb-2" key={index}>
                        <div className="flex mb-1">
                          <span className="mr-3 font-bold">{cell.columnHeader}:</span>
                          <input
                            type="text"
                            value={cell.cellValue}
                            onChange={(e) => updateCellValue(index, e.target.value)}
                            className={` flex-1 px-4 border  ${cell.isTicked ? 'bg-gray-700/30 cursor-not-allowed border-gray-700/30' : ' border-gray-600'}`}
                            disabled={cell.isTicked} 
                          />
                        </div>
                        <div>
                          <label htmlFor={`defaultTickCheckbox-${index}`} className="mr-3">
                            Tick Icon Instead?
                          </label>
                          <input
                            type="checkbox"
