...
const TableHeader = ({ data, scrollData }) => {
  return (
    <div className="grid grid-cols-[1fr,30px,repeat(5,minmax(150px,1fr))] pt-1 px-6">
      {/* Sticky Header Column */}
      <div className="sticky top-0 left-0 z-10 pl-4 pb-2 bg-slate-50 text-gray-700 font-bold">
        {data.tableHeader}
      </div>
      {/* Sticky void column */}
...
const TableBox = ({ data, index }) => {
  ...
  return (
    <div className="bg-slate-50 py-20">
      <div className="lg:mx-auto relative max-w-screen-lg">
        <div
          className="overflow-x-scroll lg:overflow-x-hidden rounded-2xl lg:shadow-none snap-x"
          ref={scrollContainerRef}
          onScroll={handleScroll}
        >
          <div className="min-w-[1200px] bg-slate-50">
            {/* Void column on the right */}
            <div className="absolute top-0 bottom-4 right-0 w-[40px] bg-gradient-to-r from-transparent rounded-2xl via-slate-50/30 to-slate-50 pointer-events-none lg:hidden"></div>
            ...
            <div className="border-t border-gray-300" key={index}>
              {data.rowItems?.map((row, rowIndex) => (
                <div
                  key={rowIndex}
                  className="grid grid-cols-[1fr,30px,repeat(5,minmax(150px,1fr))] h-[50px] px-6 hover:bg-cyan-400/5 snap-start"
                >
