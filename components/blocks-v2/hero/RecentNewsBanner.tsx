...
export const RecentNewsBanner = ({
  ...
  const BadgeIconComponent = recentNewsBanner?.badge?.icon
    ? IconOptions[recentNewsBanner.badge.icon]
    : null;
  const TitleIconComponent = recentNewsBanner?.titleIcon
    ? IconOptions[recentNewsBanner.titleIcon]
    : null;
  const badgePositionType: BadgePositionType =
    recentNewsBanner.badge?.position === 'top left' ||
    recentNewsBanner.badge?.position === 'top right'
      ? 'top'
      : 'inline';
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          {recentNewsBanner.badge?.text &&
            recentNewsBanner.badge?.position &&
            badgePositionType === 'inline' && (
              <Badge
                color={recentNewsBanner.badge?.color as BadgeColor}
                size="medium"
              ...
              >
                {BadgeIconComponent && (
                  <BadgeIconComponent className="w-5 h-5" />
                )}
                {recentNewsBanner.badge?.text}
              </Badge>
            )}

          {TitleIconComponent && <TitleIconComponent className="size-5 ml-2" />}

          <AnimatedShinyText
            className={cn(
              'flex items-center @max-lg:text-xs gap-2 text-sm',
              badgePositionType === 'inline' &&
                recentNewsBanner.badge?.position === 'left' &&
                'pr-2',
              badgePositionType === 'inline' &&
                recentNewsBanner.badge?.position === 'right' &&
                'pl-2',
              (badgePositionType === 'top' ||
                !recentNewsBanner.badge?.position) &&
                'px-2',
